...
assert(landingText.includes("ЖИН ХАСАХ АМАРХАН БОЛЛОО"));
assert(landing.includes("Та жин хасахад тань юу саад болж, яагаад хэцүү болгодогоо мэддэг гэж боддог уу? Таныг заримдаа дэглэмээ зөрчиж хооллох, хааяа нэг амттан сэмээрхэн идчихдэг, зарим хоолыг хэтрүүлчихдэг сэтгэлзүйн шалтгаануудаа та сайн мэдэж байгаа. Гэхдээ таныг далдуур удирдаж буй сэтгэлзүйн дадал зуршлууд хоорондоо нийлэхээрээ ямар үр дүнд хүргэдэгийг мэдэх үү?"));
assert(landing.includes("Жин хасахад саад болж буй сэтгэлзүйн хэв маягууд, тэдгээрийн харилцан нөлөө болон хэрхэн удирдаж, нөлөөг нь багасгах аргуудаа мэдэхийн тулд тестээ бөглөөрэй. Энэ мэдээлэл таны бүрэн тайланд нээгдэнэ."));
assert(landing.includes("Бүрэн тайлан тань яг үүнийг харуулна."));
...
assert(landing.includes(">ТЕСТЭЭ ЭХЛҮҮЛЭХ</a>"));
assert(landing.includes('href="/assessment/start"'));
assert(landing.includes("Тест үнэгүй · Хувийн тайлан 19,900₮"));
assert(!landing.includes("Эхний хувийн үр дүн"));
...
assert(!landing.includes("Үнэ: 19,900₮"));
const landingHero = /<section class="hero"[^>]*>[\s\S]*?<\/section>/.exec(landing)?.[0] || "";
assert(landingHero);
assert(landingHero.includes("19,900₮"));
assert.equal(app.PRODUCT.amount, 19900);
...
assert(appSource.includes('trackEvent("start_cta_clicked", "", `start_cta_clicked:${Date.now()}`)'), "landing start CTA analytics event remains wired");
assert(appSource.includes('if (route === "assessmentContact") trackEvent("payment_preparation_viewed"'), "historical payment preparation analytics remains isolated from the free start route");
...
assert(completion.includes("Таны хариултад тулгуурласан хувийн тайлан бэлэн боллоо"));
assert(completion.includes("Бүрэн тайлангаас та:"));
assert(completion.includes("БҮРЭН ТАЙЛАНГАА НЭЭХ"));
assert(completion.includes("19,900₮"));
assert.equal((completion.match(/<li>/g) || []).length, 3);
...
assert(transitioningCompletion.includes("НЭХЭМЖЛЭЛ ҮҮСГЭЖ БАЙНА…"));
...
assert(paymentPage.includes("Үнэ: 19,900₮"));
assert(paymentPage.includes("19,900₮-ийн QPay нэхэмжлэл үүсгэх"));
