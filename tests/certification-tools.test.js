...
(async () => {
  ...
  assert.match(gatewaySource, /reportVersionActions\.has[\s\S]*mapRecordKeys\(operation, snakeKey\)/, "report-version RPC root fields must be normalized to PostgreSQL snake_case");
  assert.match(gatewaySource, /reportVersionActions[\s\S]*get_active_report_snapshot[\s\S]*activate_report_snapshot_version/);
  assert.match(gatewaySource, /: \{ \.\.\.operation \}/, "established custom RPC root contracts must remain unchanged");
