...
function assertSafeRenderedOutput(html, label) {
...
withLocalStorageMutationSpy(() => {
  ...
  assertRenderDoesNotMutateMockBackend(() => {
    const html = _internal.renderReport();
    const text = normalize(html);
    assertSafeRenderedOutput(html, "paid output");
    assert(text.includes("Тэр мөчид хоол ямар мэдрэмж өгч байна вэ"), "paid output should show paid report depth");
    assert(text.includes("14 хоногийн туршилт"), "paid output should show paid experiment depth");
    assert(!text.includes("Бүрэн тайлан нээх 29,000₮"), "paid output should not show locked payment state");
  }, "paid output");
  ...
  assertRenderDoesNotMutateMockBackend(() => {
    ...
    assert(text.includes("Ярилцах товч нэгтгэл"), "professional output should keep professional safety summary visible");
