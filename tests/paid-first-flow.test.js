...
(async () => {
  ...
  const provider = { async createInvoice() { creates += 1; return { invoiceId: "INV-PREPAID-1", qrText: "stub", urls: [{ name: "Тест банк", link: "https://example.test/pay" }] }; },
    async checkPayment() { return { rows: [{ payment_status: "PAID", payment_amount: 19900, payment_id: "provider-paid-1" }] }; } };
  ...
  for (const copy of ["Тест үнэлгээ болон бүрэн тайлангаа нээх", "Тестийн хариултад тулгуурлан жин хасахад тань нөлөөлж буй сэтгэлзүйн болон зан үйлийн хэв маягийг тайлбарлана.", "19,900₮", "QPay-аар 19,900₮ төлөөд тестээ эхлүүлэх"]) assert(prep.includes(copy), copy);
