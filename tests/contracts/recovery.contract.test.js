...
(async () => {
  ...
  const recoveredAt = new Date(resentTime.getTime() + 1_000);
  const recovered = await confirmRecovery(database, { recoveryId: resent.recoveryId, code: newest.code }, recoveredAt);
  assert.equal(recovered.nextRoute, "/assessment/questions");
  const confirmHandlerSource = fs.readFileSync(require.resolve("../../netlify/functions/weight-recovery-confirm.js"), "utf8");
  assert(confirmHandlerSource.includes("nextRoute: result.nextRoute"), "recovery handler returns the authoritative server route");
  const cookie = recovered.cookie.split(";")[0];
  const session = await authenticateSession(database, { headers: { cookie } }, true, recoveredAt);
