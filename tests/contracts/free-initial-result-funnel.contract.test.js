...
(async () => {
  ...
  assert.equal(initial.price, 19900);
  ...
  assert.deepEqual(countOnlyProjected, { schemaVersion: INITIAL_RESULT_SCHEMA_VERSION, mode: "sealed", price: 19900, currency: "MNT" });
  ...
  const provider = {
    async createInvoice(input) {
      providerCreates += 1;
      assert.equal(input.amount, 19900);
      return { invoiceId: "free-contract-invoice", qrText: "safe", urls: [] };
    },
    async checkPayment() {
      return { rows: [{ payment_id: "provider-free-contract", payment_status: "PAID", payment_amount: 19900 }] };
    }
  };
