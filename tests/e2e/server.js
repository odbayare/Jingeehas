...
const endpoints = {
  ...
  "admin-analytics-daily": async (_body, response) => json(response, 200, { timeZone: "Asia/Ulaanbaatar",
    ...
    paidFirstFunnel: { landing: { eligibleVisitors: 10, ctaSessions: 8, preparationSessions: 6, ctaToPreparationSessions: 6, directPreparationSessions: 0 }, checkout: { preparationSessions: 6, paymentCtaSessions: 5, preparationToPaymentCtaSessions: 5, assessmentShellsCreated: 4, assessmentShellCreateFailures: 1, invoiceCreateAttempts: 4, invoicesCreated: 4, invoiceCreateFailures: 0 }, operational: { paymentPendingAssessments: 1, activePendingInvoices: 1, expiredUnpaidInvoices: 0, confirmedPayments: 3, activeEntitlements: 3, revenueMnt: 29700 }, daily: [{ date: "2026-07-19", newVisitors: 10, ctaSessions: 8, preparationSessions: 6, invoicesCreated: 4, paymentsConfirmed: 3, assessmentsStarted: 3, assessmentsCompleted: 2, reportsOpened: 2, revenueMnt: 29700 }] } }),
  ...
  "weight-assessment-save": async (body, response, request) => { stats.assessmentSave += 1; const ids = Object.keys(body.answers || {}); const delayed = String(request.headers.referer || "").includes("delaySave=1"); await new Promise(resolve => setTimeout(resolve, delayed ? 2200 : ids.includes("Q-AGE") ? 250 : 20)); json(response, 200, { assessmentId: "wa-e2e", status: "in_progress", savedQuestionIds: ids }); },
