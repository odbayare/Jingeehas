...
const endpoints = {
  ...
  "weight-assessment-create": async (_body, response, request) => { assessmentStatus = "payment_pending"; const previewBypass = String(request.headers.cookie || "").includes("jingeehas_owner_preview=preview-e2e"); json(response, 201, { assessmentId: "wa-e2e", status: assessmentStatus, commercialFlowVersion: "prepaid_v2", questionnaireVersion: questions.QUESTIONNAIRE_VERSION, previewBypass }); },
  ...
  "weight-assessment-questions": async (_body, response) => { assessmentStatus = "in_progress"; json(response, 200, { assessmentId: "wa-e2e", status: assessmentStatus, startedAt: "2026-07-21T08:00:00.000Z", questionnaireVersion: questions.QUESTIONNAIRE_VERSION }); },
  ...
  "weight-question-progress": async (body, response, request) => { const ownerPreview = String(request.headers.cookie || "").includes("jingeehas_owner_preview=preview-e2e");
    if (ownerPreview) return json(response, 200, { recorded: false, excluded: true });
  ...
  "weight-session-state": async (_body, response, request) => { const paid = stats.qpayCheck > 0; const hasPayment = stats.paymentRows > 0; const directReport = String(request.headers.referer || "").includes("/report"); const complete = assessmentStatus === "complete" || directReport; const status = complete ? "complete" : assessmentStatus; json(response, 200, { assessment: { assessmentId: "wa-e2e", status, safetyRoute: null, commercialFlowVersion: "prepaid_v2", questionnaireVersion: questions.QUESTIONNAIRE_VERSION }, nextRoute: complete ? "/report" : paid ? "/assessment/questions" : "/assessment/payment", payment: hasPayment ? { status: paid ? "paid" : "pending", paymentId: "wp-e2e" } : null, answers: {}, report: complete ? { assessmentId: "wa-e2e", reportMode: "sufficient", safetyRoute: null, initialView: {}, fullReport: selectedReport(request), entitled: true } : null }); },
