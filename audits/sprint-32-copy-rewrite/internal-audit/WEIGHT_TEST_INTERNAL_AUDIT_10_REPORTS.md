# Sprint 32 internal audit export

This file is internal only. It may include route, verdict, answer IDs, checklist, and simulated feedback.

Summary: 10 PASS / 0 PARTIAL / 0 FAIL
Recommendation: READY FOR CONTROLLED HUMAN RETEST

## user-01 - Орой тэнхээ дуусдаг хэрэглэгч

Route: deep
Verdict: PASS
Primary: Executive Load Failure
Checklist: route=true, clear=true, safe=true, bannedClean=true

### Persona

Ажилтай, гэр бүлтэй. Өдөр гайгүй барьдаг ч орой ядарч хоол захиалдаг. Хоолны цаг өдөр бүр өөр. Нойр 5-6 цаг.

### Selected answer summary

- S1-C01: 38
- S1-W02: Хөдөлгөөн багассан, Нойр муудсан
- S1-M01: Хоолны цаг өдөр бүр өөр
- S1-F01: Ядарсан, Хамгийн амар сонголт тэр байсан
- S1-L01: Бараг өдөр бүр
- S1-L02: Хоол захиалах, Гэрт байсан амар сонголт
- S1-L03: Цаг, Ядаргаа, Юу хийхээ шийдэх
- S1-N01: 4-6 цаг
- S1-S04: Үгүй

### User-facing report text

Тайлан
Таны тайлан бэлэн боллоо
Доорх нь таны өгсөн мэдээллээс гарсан эхний тайлбар. Өөрийгөө буруутгах гэж биш, өдөрт хаана хэцүү болдгийг тайван харах гэж уншаарай.
Товч хариу
1. Таны гол гацдаг мөч
Орой болоход “юу идэх вэ?” гэж бодох хүртэл хүнд болдог.
2. Энэ юу гэсэн үг вэ?
- Та юу хийхээ мэдэхгүйдээ биш.
- Өдөржин олон зүйл хийсний дараа хоол сонгох хүртэл хүнд болдог.
- Тэр үед хамгийн амар зүйл түрүүлдэг.
3. Эхлээд хийх нэг жижиг зүйл
Орой биш, эртхэн 2 бэлэн хоолны сонголт тогтоо.
4. Одоогоор түр болгоомжлох зүйл
Ядарсан орой өөрөөсөө дахин олон шийдвэр шаардах төлөвлөгөөнөөс түр зайлсхий.
Дэлгэрэнгүй тайлан
Доорх хэсэг илүү дэлгэрэнгүй. Эхний хариу хангалттай санагдвал бүгдийг нь унших албагүй.
Гол зураг
Орой болоход “юу идэх вэ?” гэж бодох хүртэл хүнд болдог.
Өдөржин ажил, гэр, хүмүүс, шийдэх зүйлс ар араасаа өнгөрсөн байна.
Тийм үед бэлэн хоол, захиалга, гарын дорх зууш түрүүлж харагддаг.
Тэр үед хоол танд юу өгч байсан байж болох вэ?
Тэр үед хоол зүгээр нэг хоол биш байсан байж болно. Түр амрах, тэнхээ орох, эсвэл өөртөө жаахан юм өгсөн мэдрэмж төрдөг.
- Дахин юм бодохгүй хэсэг амрах
- Оройн ядаргааг хурдан намдаах
- Гарын дор байгаа зүйлээр түр амсхийх
Давтагддаг тойрог
Өдөржин олон зүйл шийдэж өнгөрнө ↓
Орой “юу идэх вэ?” гэх асуулт хүртэл хүнд санагдана ↓
Бэлэн хоол түрүүлж нүдэнд тусна ↓
Захиалга эсвэл зууш руу амархан орно ↓
Хэсэг амарсан мэт болно ↓
Дараа нь “маргааш өөрөөр хийнэ” гэж бодогдоно ↓
Маргааш орой бэлэн зам байхгүй бол энэ тойрог буцаж ирнэ
Үүнийг юунаас харсан бэ?
Энэ тайлбар таны өгсөн мэдээлэл, бичсэн богино тайлбар дээр тулгуурласан. 7 хоног тэмдэглэвэл аль өдөр, ямар үед илүү хэцүү болдгийг сайн ялгана.
Асуудал яг юу биш вэ?
Гол нь мэдэхгүйдээ биш. Орой тэнхээ дууссан үед энгийн зүйл ч хүнд болдог. Тийм үед хамгийн амар хоол түрүүлж харагддаг.
Одоогоор болгоомжлох зүйлс
- Орой олон сонголттой үлдэх
- Хэт төвөгтэй хоол бэлдэх
- Өдөр бүр төгс хийх гэж шахах
- Оройн хоолыг шууд бүрэн хорих
Эхний жижиг өөрчлөлт
Эхний жижиг өөрчлөлт бол орой биш, эртхэн хоёр бэлэн хоол сонгох. Орой ирэхэд шууд ашиглахад амар байх ёстой.
14 хоногийн туршилт
Эхний 3 өдөр: орой идэж болох 2 бодит хоолоо сонго. Төгс байх албагүй. Амьдралд таардаг байх нь чухал.
4-10 дахь өдөр: ядарсан орой тэр хоёрын аль нэгийг хэрэглэ. Захиалга авсан өдөр ч өөрийгөө шийтгэхгүй.
11-14 дахь өдөр: аль хоол хамгийн бага бодол шаардсаныг тэмдэглэ.
Хэрвээ нэг өдөр алгасвал: маргааш илүү чанга барих биш, бэлэн хоёр сонголт руугаа буц.
7 хоногоор нарийвчлах
Үүнийг өдөр тутамд илүү сайн ялгахыг хүсвэл 7 хоногийн богино тэмдэглэл тусална.
- Орой хоол бодох хүнд үе ямар өдөр илүү хүчтэй болдгийг тэмдэглэлээр ялгана.
- Нойр/тэнхээ ямар өдөр илүү хүчтэй болдгийг тэмдэглэлээр ялгана.
- Идэх хүсэл гарахын өмнөх 30 минут, оройн тэнхээ, хоол харагдах зэрэг ойрын нөхцөлийг тэмдэглэнэ.
7 хоногоор нарийвчлах
Дотоод туршилтын хувилбар — энэ шатанд бодит төлбөр авахгүй.
Туршилтын санал асуулга
Та тайлангаа уншаад доорх асуултад үнэнээр нь хариулаарай. Бид энэ мэдээллийг тестийн ойлгомж, найруулга, хэрэгцээг сайжруулахад ашиглана.
Тест бөглөх явцад эвгүй, ичмээр, шүүсэн мэдрэмж төрсөн үү?
Үгүй
Бага зэрэг
Тийм
Аль хэсэг дээр?
Асуултууд ойлгомжтой байсан уу?
Маш ойлгомжтой
Ерөнхийдөө ойлгомжтой
Зарим нь ойлгомжгүй
Ихэнх нь ойлгомжгүй
Ойлгомжгүй санагдсан асуулт байвал бичнэ үү.
Тайлан таны нөхцөлтэй хэр нийцсэн бэ?
1 = огт нийцээгүй, 10 = маш сайн нийцсэн
Тайлангийн эхний “Товч хариу” хэсэг ойлгомжтой байсан уу?
Тийм, шууд ойлгосон
Ерөнхийдөө ойлгосон
Дахин уншиж байж ойлгосон
Ойлгоогүй
Ойлгомжгүй байсан хэсгийг бичнэ үү.
Тайлан уншихад “намайг ойлгож байна” гэсэн мэдрэмж төрсөн үү?
Тийм
Зарим хэсэг дээр
Үгүй
Яагаад?
Тайлангаас танд хэрэгтэй шинэ өнцөг, шинэ ойлголт гарсан уу?
Тийм
Бага зэрэг
Үгүй
Ямар хэсэг?
Тайлан хэт ерөнхий, AI шиг, эсвэл худлаа санагдсан хэсэг байсан уу?
Үгүй
Тийм
Аль хэсэг?
Тайлангийн хэл найруулга ямар санагдсан бэ?
Байгалийн монгол хэлтэй
Зарим хэсэг хиймэл
Хэт албархуу
Хэт зөөлөн/бөөрөнхий
Засах санал:
Энэ тайланг 29,000 төгрөгөөр авахад үнэ цэнтэй санагдах уу?
Тийм
Магадгүй
Үгүй
Яагаад?
Хамгийн хэрэгтэй санагдсан хэсэг юу байсан бэ?
Хамгийн засмаар санагдсан хэсэг юу байсан бэ?
Санал илгээх
Саналын экспорт
Сонголт руу буцах Шинээр эхлэх

### Virtual human feedback

- Simple result: Тийм, шууд ойлгосон
- Fit: 8/10
- Felt understood: Тийм
- AI/generic feeling: Үгүй

---

## user-02 - Стрессээр иддэг хэрэглэгч

Route: deep
Verdict: PASS
Primary: Food-as-Regulation System
Checklist: route=true, clear=true, safe=true, bannedClean=true

### Persona

Оффисын ажил. Санаа зовнил, уур, ажлын дарамтын дараа идэх хүсэл нэмэгддэг. Идсэний дараа түр тайвширдаг, дараа нь гэмшдэг.

### Selected answer summary

- S1-E01: Ихэвчлэн тэгдэг
- S1-E02: Стресс, Уур, Санаа зовнил
- S1-E03: Түр тайвширдаг
- S1-F01: Тайвширмаар байсан
- S1-F02: Түр гайгүй болоод гэмшдэг
- S1-S04: Үгүй

### User-facing report text

Тайлан
Таны тайлан бэлэн боллоо
Доорх нь таны өгсөн мэдээллээс гарсан эхний тайлбар. Өөрийгөө буруутгах гэж биш, өдөрт хаана хэцүү болдгийг тайван харах гэж уншаарай.
Товч хариу
1. Таны гол гацдаг мөч
Стресс ихтэй үед хоол танд түр амрах газар шиг болж байна.
2. Энэ юу гэсэн үг вэ?
- Тэр үед хоол зөвхөн өлсөлт дарах зүйл биш байна.
- Дотор давчдах, санаа зовох үед хоол хэсэг тайвшруулах шиг болдог.
- Дараа нь гэмших мэдрэмж нэмэгдвэл энэ тойрог улам хүндэрдэг.
3. Эхлээд хийх нэг жижиг зүйл
Идэхээс өмнө 60 секунд “би өлсөж байна уу, амсхийх газар хайж байна уу?” гэж ялга.
4. Одоогоор түр болгоомжлох зүйл
Стресс ихтэй өдөр өөрийгөө зөвхөн тэвч гэж шахахаас түр зайлсхий.
Дэлгэрэнгүй тайлан
Доорх хэсэг илүү дэлгэрэнгүй. Эхний хариу хангалттай санагдвал бүгдийг нь унших албагүй.
Гол зураг
Стресс ихтэй үед хоол түр амрах газар шиг санагдаж байна.
Дотор давчдах, санаа зовох, уур хүрэх үед идэхэд хэсэг намддаг.
Дараа нь гэмших мэдрэмж ирвэл тэр амралт удаан тогтохгүй.
Тэр үед хоол танд юу өгч байсан байж болох вэ?
Тэр үед хоол зүгээр нэг хоол биш байсан байж болно. Түр амрах, тэнхээ орох, эсвэл өөртөө жаахан юм өгсөн мэдрэмж төрдөг.
- Сэтгэл тавгүй үед хэсэг намдах
- Дотор давчдах мэдрэмжийг зөөллөх
- Яг тэр үед амсхийх зай гаргах
Давтагддаг тойрог
Стресс нэмэгдэнэ ↓
Дотор давчдаж, тавгүй болно ↓
Хоол хамгийн ойрын амралт шиг санагдана ↓
Идсэний дараа хэсэг намдана ↓
Дараа нь гэмших эсвэл санаа зовох мэдрэмж орж ирнэ ↓
Стресс хэвээр байвал хоол дараагийн удаа дахиад ойрхон санагдана
Үүнийг юунаас харсан бэ?
Энэ тайлбар таны өгсөн мэдээлэл, бичсэн богино тайлбар дээр тулгуурласан. 7 хоног тэмдэглэвэл аль өдөр, ямар үед илүү хэцүү болдгийг сайн ялгана.
Асуудал яг юу биш вэ?
Асуудал хоолондоо биш. Стресс өндөр үед хоол хамгийн ойр амсхийх газар шиг санагдаж байна.
Одоогоор болгоомжлох зүйлс
- Илүү хатуу хоолны дэглэмийг бэлэн шийдэл болгох
- Стрессийн мөчид зөвхөн тэвчих гэж шахах
- Гэмшлээр өөрийгөө хөдөлгөх
- Мэдрэмжээ огт нэрлэхгүй орхих
Эхний жижиг өөрчлөлт
Эхний жижиг өөрчлөлт бол хоолноос өмнө өөрийгөө хорих биш. 60 секунд зогсоод “өлсөж байна уу, амрах газар хайж байна уу?” гэж ялга.
14 хоногийн туршилт
Эхний 3 өдөр: идэх хүсэл стрессийн дараа гарсан эсэхийг л тэмдэглэ. Засах гэж бүү оролд.
4-10 дахь өдөр: хоолноос өмнө 60 секунд амьсгаа авах, ус уух, алхах, хүн рүү бичихээс нэгийг турш.
11-14 дахь өдөр: аль богино амралт хамгийн бодитой байсныг сонго.
Хэрвээ нэг өдөр алгасвал: тэр өдрийг бүтэлгүйтэл гэж үзэхгүй. Дараагийн стрессийн мөч дээр дахин 60 секундээс эхэл.
7 хоногоор нарийвчлах
Үүнийг өдөр тутамд илүү сайн ялгахыг хүсвэл 7 хоногийн богино тэмдэглэл тусална.
- Стрессийн дараах идэлт ямар өдөр илүү хүчтэй болдгийг тэмдэглэлээр ялгана.
- Идэх хүсэл гарахын өмнөх 30 минут, оройн тэнхээ, хоол харагдах зэрэг ойрын нөхцөлийг тэмдэглэнэ.
7 хоногоор нарийвчлах
Дотоод туршилтын хувилбар — энэ шатанд бодит төлбөр авахгүй.
Туршилтын санал асуулга
Та тайлангаа уншаад доорх асуултад үнэнээр нь хариулаарай. Бид энэ мэдээллийг тестийн ойлгомж, найруулга, хэрэгцээг сайжруулахад ашиглана.
Тест бөглөх явцад эвгүй, ичмээр, шүүсэн мэдрэмж төрсөн үү?
Үгүй
Бага зэрэг
Тийм
Аль хэсэг дээр?
Асуултууд ойлгомжтой байсан уу?
Маш ойлгомжтой
Ерөнхийдөө ойлгомжтой
Зарим нь ойлгомжгүй
Ихэнх нь ойлгомжгүй
Ойлгомжгүй санагдсан асуулт байвал бичнэ үү.
Тайлан таны нөхцөлтэй хэр нийцсэн бэ?
1 = огт нийцээгүй, 10 = маш сайн нийцсэн
Тайлангийн эхний “Товч хариу” хэсэг ойлгомжтой байсан уу?
Тийм, шууд ойлгосон
Ерөнхийдөө ойлгосон
Дахин уншиж байж ойлгосон
Ойлгоогүй
Ойлгомжгүй байсан хэсгийг бичнэ үү.
Тайлан уншихад “намайг ойлгож байна” гэсэн мэдрэмж төрсөн үү?
Тийм
Зарим хэсэг дээр
Үгүй
Яагаад?
Тайлангаас танд хэрэгтэй шинэ өнцөг, шинэ ойлголт гарсан уу?
Тийм
Бага зэрэг
Үгүй
Ямар хэсэг?
Тайлан хэт ерөнхий, AI шиг, эсвэл худлаа санагдсан хэсэг байсан уу?
Үгүй
Тийм
Аль хэсэг?
Тайлангийн хэл найруулга ямар санагдсан бэ?
Байгалийн монгол хэлтэй
Зарим хэсэг хиймэл
Хэт албархуу
Хэт зөөлөн/бөөрөнхий
Засах санал:
Энэ тайланг 29,000 төгрөгөөр авахад үнэ цэнтэй санагдах уу?
Тийм
Магадгүй
Үгүй
Яагаад?
Хамгийн хэрэгтэй санагдсан хэсэг юу байсан бэ?
Хамгийн засмаар санагдсан хэсэг юу байсан бэ?
Санал илгээх
Саналын экспорт
Сонголт руу буцах Шинээр эхлэх

### Virtual human feedback

- Simple result: Ерөнхийдөө ойлгосон
- Fit: 8/10
- Felt understood: Тийм
- AI/generic feeling: Тийм

---

## user-03 - Хэт чанга эхлээд нурдаг хэрэглэгч

Route: deep
Verdict: PASS
Primary: Control-Collapse Cycle
Checklist: route=true, clear=true, safe=true, bannedClean=true

### Persona

Даваагаас эхэлнэ гэж боддог. Нэг удаа хазайвал өнөөдөр өнгөрлөө гэж мэдэрдэг. Маргаашаас илүү чанга барина.

### Selected answer summary

- S1-W03: Бараг бүх оролдлогоос хойш
- S1-W04: Калори тоолох, Мацаг, Орой хоол идэхгүй
- S1-W06: Өнөөдөр өнгөрлөө, маргаашаас
- S1-F02: Одоо бүх юм дууссан
- S1-X03: Маш хүчтэй
- S1-S04: Үгүй

### User-facing report text

Тайлан
Таны тайлан бэлэн боллоо
Доорх нь таны өгсөн мэдээллээс гарсан эхний тайлбар. Өөрийгөө буруутгах гэж биш, өдөрт хаана хэцүү болдгийг тайван харах гэж уншаарай.
Товч хариу
1. Таны гол гацдаг мөч
Нэг удаа төлөвлөгөө зөрөхөд “өнөөдөр өнгөрлөө” гэж мэдрэгддэг.
2. Энэ юу гэсэн үг вэ?
- Асуудал ганц удаа хазайсандаа биш.
- Тэр мөчөөс хойш дараагийн хоолноос хэвийн үргэлжлэх зам бүдгэрч байна.
- Маргаашаас илүү чанга барина гэсэн бодол тойргийг дахин эхлүүлж магадгүй.
3. Эхлээд хийх нэг жижиг зүйл
Нэг хазайлтын дараа хэрэглэх “дараагийн хоолноос хэвийн үргэлжлүүлэх” дүрэмтэй бол.
4. Одоогоор түр болгоомжлох зүйл
Нэг алдааны дараа бүхнийг дахин эхлүүлэх хатуу дэглэмээс түр зайлсхий.
Дэлгэрэнгүй тайлан
Доорх хэсэг илүү дэлгэрэнгүй. Эхний хариу хангалттай санагдвал бүгдийг нь унших албагүй.
Гол зураг
Нэг удаа хазайхаар бүх өдөр дуусчихсан юм шиг санагддаг.
Тэр мөчөөс хойш “өнөөдөр өнгөрлөө” гэсэн бодол орж ирнэ.
Дараагийн хоолноос хэвийн үргэлжлэх зам бүдгэрдэг.
Тэр үед хоол танд юу өгч байсан байж болох вэ?
Тэр үед хоол зүгээр нэг хоол биш байсан байж болно. Түр амрах, тэнхээ орох, эсвэл өөртөө жаахан юм өгсөн мэдрэмж төрдөг.
- “Нэгэнт алдсан” гэсэн дарамтаас түр холдох
- Хэт чанга дүрмийн эсрэг түр сулрах
- Өөрийгөө буруутгах мэдрэмжээс хэсэг зай авах
Давтагддаг тойрог
Шинэ төлөвлөгөө хэт чанга эхэлнэ ↓
Бага зэрэг хазайх мөч гарна ↓
“Өнөөдөр өнгөрлөө” гэж бодогдоно ↓
Дараагийн хоолноос хэвийн үргэлжлэх зам харагдахгүй болно ↓
Илүү их идэх эсвэл бүхлээр нь орхих хандлага гарна ↓
Дараа нь гэмшил нэмэгдэнэ ↓
Маргаашаас илүү чанга барина гэж бодно
Үүнийг юунаас харсан бэ?
Энэ тайлбар таны өгсөн мэдээлэл, бичсэн богино тайлбар дээр тулгуурласан. 7 хоног тэмдэглэвэл аль өдөр, ямар үед илүү хэцүү болдгийг сайн ялгана.
Асуудал яг юу биш вэ?
Асуудал нэг удаа төлөвлөгөө алдсандаа биш. Гол нь буцаад хэвийн хоол руу орох жижиг зам харагдахгүй болдог.
Одоогоор болгоомжлох зүйлс
- Төгс төлөвлөгөө
- Нэг алдаа гарвал бүхнийг дахин эхлүүлэх сорил
- Ичгүүрээр шахдаг хяналт
- Алдаагаа нөхөх гэж маргааш нь илүү чангаруулах
Эхний жижиг өөрчлөлт
Эхний жижиг өөрчлөлт бол төгс төлөвлөгөө биш. Нэг хазайлтын дараа хэрэглэх “дараагийн хоолноос хэвийн үргэлжлүүлэх” дүрэмтэй болох.
14 хоногийн туршилт
Эхний 3 өдөр: “70% хангалттай” гэсэн нэг өгүүлбэр бичээд өдөр бүр харагдах газар тавь.
4-10 дахь өдөр: төлөвлөгөө зөрсөн мөч гарвал дараагийн хоолыг шийтгэлгүй хэвийн хоол болго. Мацгаар засах гэж оролдохгүй.
11-14 дахь өдөр: “өнөөдөр өнгөрлөө” бодол гарсан эсэх, дараагийн хоолноос буцаж чадсан эсэхээ тэмдэглэ.
Хэрвээ нэг өдөр алгасвал: маргааш илүү чанга барихгүй. Зүгээр дараагийн хоолноос эхэл.
7 хоногоор нарийвчлах
Үүнийг өдөр тутамд илүү сайн ялгахыг хүсвэл 7 хоногийн богино тэмдэглэл тусална.
- Хэвийн үргэлжлүүлэхэд хэцүү мөч ямар өдөр илүү хүчтэй болдгийг тэмдэглэлээр ялгана.
- Хоолны зай уртсах үе ямар өдөр илүү хүчтэй болдгийг тэмдэглэлээр ялгана.
- Идэх хүсэл гарахын өмнөх 30 минут, оройн тэнхээ, хоол харагдах зэрэг ойрын нөхцөлийг тэмдэглэнэ.
7 хоногоор нарийвчлах
Дотоод туршилтын хувилбар — энэ шатанд бодит төлбөр авахгүй.
Туршилтын санал асуулга
Та тайлангаа уншаад доорх асуултад үнэнээр нь хариулаарай. Бид энэ мэдээллийг тестийн ойлгомж, найруулга, хэрэгцээг сайжруулахад ашиглана.
Тест бөглөх явцад эвгүй, ичмээр, шүүсэн мэдрэмж төрсөн үү?
Үгүй
Бага зэрэг
Тийм
Аль хэсэг дээр?
Асуултууд ойлгомжтой байсан уу?
Маш ойлгомжтой
Ерөнхийдөө ойлгомжтой
Зарим нь ойлгомжгүй
Ихэнх нь ойлгомжгүй
Ойлгомжгүй санагдсан асуулт байвал бичнэ үү.
Тайлан таны нөхцөлтэй хэр нийцсэн бэ?
1 = огт нийцээгүй, 10 = маш сайн нийцсэн
Тайлангийн эхний “Товч хариу” хэсэг ойлгомжтой байсан уу?
Тийм, шууд ойлгосон
Ерөнхийдөө ойлгосон
Дахин уншиж байж ойлгосон
Ойлгоогүй
Ойлгомжгүй байсан хэсгийг бичнэ үү.
Тайлан уншихад “намайг ойлгож байна” гэсэн мэдрэмж төрсөн үү?
Тийм
Зарим хэсэг дээр
Үгүй
Яагаад?
Тайлангаас танд хэрэгтэй шинэ өнцөг, шинэ ойлголт гарсан уу?
Тийм
Бага зэрэг
Үгүй
Ямар хэсэг?
Тайлан хэт ерөнхий, AI шиг, эсвэл худлаа санагдсан хэсэг байсан уу?
Үгүй
Тийм
Аль хэсэг?
Тайлангийн хэл найруулга ямар санагдсан бэ?
Байгалийн монгол хэлтэй
Зарим хэсэг хиймэл
Хэт албархуу
Хэт зөөлөн/бөөрөнхий
Засах санал:
Энэ тайланг 29,000 төгрөгөөр авахад үнэ цэнтэй санагдах уу?
Тийм
Магадгүй
Үгүй
Яагаад?
Хамгийн хэрэгтэй санагдсан хэсэг юу байсан бэ?
Хамгийн засмаар санагдсан хэсэг юу байсан бэ?
Санал илгээх
Саналын экспорт
Сонголт руу буцах Шинээр эхлэх

### Virtual human feedback

- Simple result: Тийм, шууд ойлгосон
- Fit: 9/10
- Felt understood: Тийм
- AI/generic feeling: Үгүй

---

## user-04 - Хоол холдоход хэтрүүлдэг хэрэглэгч

Route: deep
Verdict: PASS
Primary: Hunger-Safety / Scarcity Protection
Checklist: route=true, clear=true, safe=true, bannedClean=true

### Persona

Хоолны зай уртсахад орой хэт өлсдөг. Дараа өлсөхөөс хамгаалж илүү иддэг. Хүнд биеийн шинжгүй.

### Selected answer summary

- S1-M01: Өдрийн хоол алгасдаг
- S1-M02: Бараг өдөр бүр
- S1-M03: Бараг дандаа тэгдэг
- S1-G01: Ихэвчлэн
- S1-G02: Тийм
- S1-F01: Өлссөндөө идсэн, Дараа өлсөхөөс санаа зовсон
- S1-S04: Үгүй

### User-facing report text

Тайлан
Таны тайлан бэлэн боллоо
Доорх нь таны өгсөн мэдээллээс гарсан эхний тайлбар. Өөрийгөө буруутгах гэж биш, өдөрт хаана хэцүү болдгийг тайван харах гэж уншаарай.
Товч хариу
1. Таны гол гацдаг мөч
Удаан юм идээгүй байх үед орой бие яараад эхэлдэг.
2. Энэ юу гэсэн үг вэ?
- Өдөр хоол холдох үед бие орой илүү яаралтай хариу өгч байна.
- Дараа хэзээ идэх нь тодорхойгүй санагдах үед илүү идэх нь хамгаалах оролдлого байж болно.
- Энэ нь сул тал биш. Өдөр хоолны зай хэт урт байсны дараах хариу байж болно.
3. Эхлээд хийх нэг жижиг зүйл
5 цагаас дээш зай гарахаас өмнө жижиг гүүр хоол эсвэл зууш төлөвлө.
4. Одоогоор түр болгоомжлох зүйл
Өдөр хоол алгасаад орой өөрийгөө тэс гэж шахахаас түр зайлсхий.
Дэлгэрэнгүй тайлан
Доорх хэсэг илүү дэлгэрэнгүй. Эхний хариу хангалттай санагдвал бүгдийг нь унших албагүй.
Гол зураг
Удаан юм идээгүй байх үед орой бие яараад эхэлдэг.
Дараа дахиад өлсөх вий гэж санагдахаар илүү идэхэд амархан болдог.
Энэ нь сул тал биш. Өдөр хэт урт хоосон явсны дараах хариу байж болно.
Тэр үед хоол танд юу өгч байсан байж болох вэ?
Тэр үед хоол зүгээр нэг хоол биш байсан байж болно. Түр амрах, тэнхээ орох, эсвэл өөртөө жаахан юм өгсөн мэдрэмж төрдөг.
- Дараа дахиад өлсөхөөс айхгүй болох
- Биеэ тайван болгох
- Оройн хүчтэй өлсөлтийг намдаах
Давтагддаг тойрог
Өдөр хоол холдоно ↓
Орой өлсөлт, яаралтай мэдрэмж нэмэгдэнэ ↓
“Дараа дахиад өлсөх вий” гэж бодогдоно ↓
Бэлэн байгаа хоол илүү татна ↓
Илүү их идсэний дараа бие түр тайвширна ↓
Маргааш дахин хасвал орой энэ тойрог буцаж ирнэ
Үүнийг юунаас харсан бэ?
Энэ тайлбар таны өгсөн мэдээлэл, бичсэн богино тайлбар дээр тулгуурласан. 7 хоног тэмдэглэвэл аль өдөр, ямар үед илүү хэцүү болдгийг сайн ялгана.
Асуудал яг юу биш вэ?
Асуудал орой идсэндээ биш. Өдөр хоолны зай хэт уртсахад орой бие хамгаалах гэж яардаг.
Одоогоор болгоомжлох зүйлс
- Урт мацаг
- Өдөр хоол алгасах
- “Өлсвөл тэс” гэж өөрийгөө хүчлэх
- Оройн зуушийг шууд бүрэн хорих
Эхний жижиг өөрчлөлт
Эхний жижиг өөрчлөлт бол оройг хорих биш. Өдрийн хоолны зайг хэт холдуулахгүй нэг тулгуур хоол, нэг жижиг гүүр сонголт бэлдэх.
14 хоногийн туршилт
Эхний 3 өдөр: хоол хооронд хэдэн цагийн зай гарч байгааг л тэмдэглэ.
4-10 дахь өдөр: 5 цагаас дээш зай гарахаас өмнө жижиг гүүр хоол ашигла.
11-14 дахь өдөр: оройн өлсөлт яаралтай санагдах нь багассан эсэхийг тэмдэглэ.
Хэрвээ нэг өдөр алгасвал: оройг шийтгэл болгохгүй. Дараагийн өдөр нэг тулгуур хоолноос эхэл.
7 хоногоор нарийвчлах
Үүнийг өдөр тутамд илүү сайн ялгахыг хүсвэл 7 хоногийн богино тэмдэглэл тусална.
- Хоолны зай уртсах үе ямар өдөр илүү хүчтэй болдгийг тэмдэглэлээр ялгана.
- Идэх хүсэл гарахын өмнөх 30 минут, оройн тэнхээ, хоол харагдах зэрэг ойрын нөхцөлийг тэмдэглэнэ.
7 хоногоор нарийвчлах
Дотоод туршилтын хувилбар — энэ шатанд бодит төлбөр авахгүй.
Туршилтын санал асуулга
Та тайлангаа уншаад доорх асуултад үнэнээр нь хариулаарай. Бид энэ мэдээллийг тестийн ойлгомж, найруулга, хэрэгцээг сайжруулахад ашиглана.
Тест бөглөх явцад эвгүй, ичмээр, шүүсэн мэдрэмж төрсөн үү?
Үгүй
Бага зэрэг
Тийм
Аль хэсэг дээр?
Асуултууд ойлгомжтой байсан уу?
Маш ойлгомжтой
Ерөнхийдөө ойлгомжтой
Зарим нь ойлгомжгүй
Ихэнх нь ойлгомжгүй
Ойлгомжгүй санагдсан асуулт байвал бичнэ үү.
Тайлан таны нөхцөлтэй хэр нийцсэн бэ?
1 = огт нийцээгүй, 10 = маш сайн нийцсэн
Тайлангийн эхний “Товч хариу” хэсэг ойлгомжтой байсан уу?
Тийм, шууд ойлгосон
Ерөнхийдөө ойлгосон
Дахин уншиж байж ойлгосон
Ойлгоогүй
Ойлгомжгүй байсан хэсгийг бичнэ үү.
Тайлан уншихад “намайг ойлгож байна” гэсэн мэдрэмж төрсөн үү?
Тийм
Зарим хэсэг дээр
Үгүй
Яагаад?
Тайлангаас танд хэрэгтэй шинэ өнцөг, шинэ ойлголт гарсан уу?
Тийм
Бага зэрэг
Үгүй
Ямар хэсэг?
Тайлан хэт ерөнхий, AI шиг, эсвэл худлаа санагдсан хэсэг байсан уу?
Үгүй
Тийм
Аль хэсэг?
Тайлангийн хэл найруулга ямар санагдсан бэ?
Байгалийн монгол хэлтэй
Зарим хэсэг хиймэл
Хэт албархуу
Хэт зөөлөн/бөөрөнхий
Засах санал:
Энэ тайланг 29,000 төгрөгөөр авахад үнэ цэнтэй санагдах уу?
Тийм
Магадгүй
Үгүй
Яагаад?
Хамгийн хэрэгтэй санагдсан хэсэг юу байсан бэ?
Хамгийн засмаар санагдсан хэсэг юу байсан бэ?
Санал илгээх
Саналын экспорт
Сонголт руу буцах Шинээр эхлэх

### Virtual human feedback

- Simple result: Ерөнхийдөө ойлгосон
- Fit: 8/10
- Felt understood: Тийм
- AI/generic feeling: Үгүй

---

## user-05 - Өөрийгөө хойш тавьдаг хэрэглэгч

Route: deep
Verdict: PASS
Primary: Reward-Seeking / Stimulation Compensation
Checklist: route=true, clear=true, safe=true, bannedClean=true

### Persona

Бусдын хэрэгцээ түрүүлдэг. Орой амттай зүйл өөртөө өгөх ганц жижиг шагнал мэт санагддаг.

### Selected answer summary

- S1-R02: Өдрийн төгсгөлд өөрийгөө жаахан баярлуулмаар санагдах үед
- S1-E02: Хоосон мэт мэдрэмж, Ядаргаа
- S1-F01: Өөрийгөө жаахан шагнамаар санагдсан, Ядарсан
- S1-S04: Үгүй

### User-facing report text

Тайлан
Таны тайлан бэлэн боллоо
Доорх нь таны өгсөн мэдээллээс гарсан эхний тайлбар. Өөрийгөө буруутгах гэж биш, өдөрт хаана хэцүү болдгийг тайван харах гэж уншаарай.
Товч хариу
1. Таны гол гацдаг мөч
Өдөржин өөрийгөө хойш тавьсны дараа орой амттай зүйл өөртөө өгөх нэг жижиг зүйл болдог.
2. Энэ юу гэсэн үг вэ?
- Энэ нь зүгээр амттан хүссэн асуудал биш.
- Өөрийн хоол, амралт, таатай зүйл өдөржин хойшлогдож байна.
- Орой хоол “надад ч гэсэн нэг юм хэрэгтэй” гэсэн мэдрэмжтэй холбогдож байна.
3. Эхлээд хийх нэг жижиг зүйл
Өдөр бүр өөрийн хоол эсвэл амралтын 10 минутыг үлдэгдэл цагт найдахгүй хамгаал.
4. Одоогоор түр болгоомжлох зүйл
Оройн амттай зүйл хүсэхийг шууд бүрэн хорихоос түр зайлсхий.
Дэлгэрэнгүй тайлан
Доорх хэсэг илүү дэлгэрэнгүй. Эхний хариу хангалттай санагдвал бүгдийг нь унших албагүй.
Гол зураг
Өдөржин өөрийн хоол, амралт хамгийн сүүлд үлддэг бол орой амттай зүйл “миний юм” шиг санагдаж болно.
Энэ нь зүгээр амттан хүссэн хэрэг биш.
Өдөржин хойш тавигдсан өөртөө ганц жижиг таатай мөч өгөх гэсэн оролдлого байж болно.
Тэр үед хоол танд юу өгч байсан байж болох вэ?
Тэр үед хоол зүгээр нэг хоол биш байсан байж болно. Түр амрах, тэнхээ орох, эсвэл өөртөө жаахан юм өгсөн мэдрэмж төрдөг.
- Өөртөө өгөх жижиг таатай мөч
- Өдөржин хойш тавигдсан өөрийгөө санах
- “Миний цаг” гэсэн мэдрэмж авах
Давтагддаг тойрог
Өдөр бусдын хэрэгцээ түрүүлнэ ↓
Өөрийн хоол, амралт хойш тавигдана ↓
Орой болоход хоосон, ядарсан, гомдолтой мэдрэмж хуримтлагдана ↓
Амттай зүйл “миний юм” шиг санагдана ↓
Идсэний дараа хэсэг таатай болно ↓
Дараа нь өөрийгөө буруутгах бодол орж ирж магадгүй ↓
Маргааш өөрийн хоол дахин хойшлогдвол энэ тойрог буцаж ирнэ
Үүнийг юунаас харсан бэ?
Энэ тайлбар таны өгсөн мэдээлэл, бичсэн богино тайлбар дээр тулгуурласан. 7 хоног тэмдэглэвэл аль өдөр, ямар үед илүү хэцүү болдгийг сайн ялгана.
Асуудал яг юу биш вэ?
Асуудал амттай юманд дуртайдаа биш. Өөрийн хоол, амралт өдөржин хойшлогдохоор орой амттай зүйл илүү үнэ цэнтэй санагддаг.
Одоогоор болгоомжлох зүйлс
- “Зүгээр өөртөө цаг гарга” гэсэн ерөнхий зөвлөгөө
- Өөрийн хоолыг үлдэгдэл цагт найдах
- Оройн өөрийгөө баярлуулах хэрэгцээг шууд бүрэн хорих
- Хэт хатуу хоолны дэглэм
Эхний жижиг өөрчлөлт
Эхний жижиг өөрчлөлт бол “өөртөө цаг гарга” гэсэн том зөвлөгөө биш. Өдөр бүр өөрийн хоол эсвэл амралтын 10 минутыг үлдэгдэл цагт найдахгүй хамгаалах.
14 хоногийн туршилт
Эхний 3 өдөр: өдөрт өөрийн төлөө хамгаалж болох хамгийн жижиг 10 минутыг сонго.
4-10 дахь өдөр: тэр 10 минутыг хоол, цай, алхалт, тайван суухын аль нэгээр ашигла.
11-14 дахь өдөр: өөрийн 10 минуттай өдөр оройн идэх хүсэл яаж өөрчлөгдсөнийг тэмдэглэ.
Хэрвээ нэг өдөр алгасвал: өөрийгөө буруутгахгүй. Дараагийн өдөр дахин 10 минутаас эхэл.
7 хоногоор нарийвчлах
Үүнийг өдөр тутамд илүү сайн ялгахыг хүсвэл 7 хоногийн богино тэмдэглэл тусална.
- Таатай зүйл хүсэх үе ямар өдөр илүү хүчтэй болдгийг тэмдэглэлээр ялгана.
- Орой хоол бодох хүнд үе ямар өдөр илүү хүчтэй болдгийг тэмдэглэлээр ялгана.
- Идэх хүсэл гарахын өмнөх 30 минут, оройн тэнхээ, хоол харагдах зэрэг ойрын нөхцөлийг тэмдэглэнэ.
7 хоногоор нарийвчлах
Дотоод туршилтын хувилбар — энэ шатанд бодит төлбөр авахгүй.
Туршилтын санал асуулга
Та тайлангаа уншаад доорх асуултад үнэнээр нь хариулаарай. Бид энэ мэдээллийг тестийн ойлгомж, найруулга, хэрэгцээг сайжруулахад ашиглана.
Тест бөглөх явцад эвгүй, ичмээр, шүүсэн мэдрэмж төрсөн үү?
Үгүй
Бага зэрэг
Тийм
Аль хэсэг дээр?
Асуултууд ойлгомжтой байсан уу?
Маш ойлгомжтой
Ерөнхийдөө ойлгомжтой
Зарим нь ойлгомжгүй
Ихэнх нь ойлгомжгүй
Ойлгомжгүй санагдсан асуулт байвал бичнэ үү.
Тайлан таны нөхцөлтэй хэр нийцсэн бэ?
1 = огт нийцээгүй, 10 = маш сайн нийцсэн
Тайлангийн эхний “Товч хариу” хэсэг ойлгомжтой байсан уу?
Тийм, шууд ойлгосон
Ерөнхийдөө ойлгосон
Дахин уншиж байж ойлгосон
Ойлгоогүй
Ойлгомжгүй байсан хэсгийг бичнэ үү.
Тайлан уншихад “намайг ойлгож байна” гэсэн мэдрэмж төрсөн үү?
Тийм
Зарим хэсэг дээр
Үгүй
Яагаад?
Тайлангаас танд хэрэгтэй шинэ өнцөг, шинэ ойлголт гарсан уу?
Тийм
Бага зэрэг
Үгүй
Ямар хэсэг?
Тайлан хэт ерөнхий, AI шиг, эсвэл худлаа санагдсан хэсэг байсан уу?
Үгүй
Тийм
Аль хэсэг?
Тайлангийн хэл найруулга ямар санагдсан бэ?
Байгалийн монгол хэлтэй
Зарим хэсэг хиймэл
Хэт албархуу
Хэт зөөлөн/бөөрөнхий
Засах санал:
Энэ тайланг 29,000 төгрөгөөр авахад үнэ цэнтэй санагдах уу?
Тийм
Магадгүй
Үгүй
Яагаад?
Хамгийн хэрэгтэй санагдсан хэсэг юу байсан бэ?
Хамгийн засмаар санагдсан хэсэг юу байсан бэ?
Санал илгээх
Саналын экспорт
Сонголт руу буцах Шинээр эхлэх

### Virtual human feedback

- Simple result: Тийм, шууд ойлгосон
- Fit: 8/10
- Felt understood: Тийм
- AI/generic feeling: Үгүй

---

## user-06 - Орчны дохиогоор автоматаар иддэг хэрэглэгч

Route: deep
Verdict: PASS
Primary: Cue-Conditioned Automatic Eating
Checklist: route=true, clear=true, safe=true, bannedClean=true

### Persona

Зууш ширээн дээр байвал иддэг. Захиалгын апп, хоолны зураг, үнэр нөлөөлдөг. Өлсөөгүй үед ч иддэг.

### Selected answer summary

- S1-H02: Ихэвчлэн үгүй
- S1-F01: Харагдаад эсвэл үнэртээд идмээр болсон
- S1-R02: Хоолны зураг эсвэл захиалгын апп харахад, Амт, үнэр, мэдрэмж татах үед
- S1-S04: Үгүй

### User-facing report text

Тайлан
Таны тайлан бэлэн боллоо
Доорх нь таны өгсөн мэдээллээс гарсан эхний тайлбар. Өөрийгөө буруутгах гэж биш, өдөрт хаана хэцүү болдгийг тайван харах гэж уншаарай.
Товч хариу
1. Таны гол гацдаг мөч
Зууш нүдэнд ойр байхаар гар өөрөө хүрчихдэг үе байна.
2. Энэ юу гэсэн үг вэ?
- Энд гол нь хүсэл зориг сулдаа биш.
- Ойр байгаа хоол, үнэр, зураг, захиалгын апп идэх бодлыг түрүүлж оруулдаг.
- Харагдах зүйл ойр байх тусам идэх бодол хурдан орж ирдэг.
3. Эхлээд хийх нэг жижиг зүйл
Хамгийн хүчтэй татдаг нэг зүйлийг нэг алхам холдуул.
4. Одоогоор түр болгоомжлох зүйл
Бүх хоолоо хорих гэж оролдохоос илүү нүдэнд ойр байгаа нэг зүйлээ өөрчил.
Дэлгэрэнгүй тайлан
Доорх хэсэг илүү дэлгэрэнгүй. Эхний хариу хангалттай санагдвал бүгдийг нь унших албагүй.
Гол зураг
Зууш нүдэнд ойр байхаар гар өөрөө хүрчихдэг үе байна.
Апп нээгдэх, хоолны зураг харагдах, үнэр ойртох үед өлсөөгүй байсан ч идмээр болдог.
Тэр үед хоол танд юу өгч байсан байж болох вэ?
Тэр үед хоол зүгээр нэг хоол биш байсан байж болно. Түр амрах, тэнхээ орох, эсвэл өөртөө жаахан юм өгсөн мэдрэмж төрдөг.
- Харагдсан зүйлд шууд хариу өгөх
- Бодолгүйгээр хурдан таатай мэдрэмж авах
- Гарын дор байгаа амар сонголтыг ашиглах
Давтагддаг тойрог
Зууш, үнэр, зураг эсвэл захиалгын апп ойр байна ↓
Өлсөөгүй байсан ч идэх бодол орж ирнэ ↓
“Нэгийг авчихъя” гэсэн жижиг шийдвэр хурдан гарна ↓
Идсэний дараа түр таатай мэдрэмж өгнө ↓
Тэр зүйл нүдэнд ойр хэвээр байвал дараа нь дахиад амархан хүрнэ
Үүнийг юунаас харсан бэ?
Энэ тайлбар таны өгсөн мэдээлэл, бичсэн богино тайлбар дээр тулгуурласан. 7 хоног тэмдэглэвэл аль өдөр, ямар үед илүү хэцүү болдгийг сайн ялгана.
Асуудал яг юу биш вэ?
Асуудал хүсэл зориг сулдаа биш. Нүдэнд ойр, гарын дор байгаа зүйлд хүн амархан хүрдэг.
Одоогоор болгоомжлох зүйлс
- Бүх амттай зүйлийг нэг дор хорих
- Гарын дорх зүйлийг хэвээр үлдээгээд зөвхөн тэвчих
- Өдөр бүр бүх орчноо төгс өөрчлөх гэж зүтгэх
- Өлсөөгүй идсэнээ зан чанарын алдаа гэж үзэх
Эхний жижиг өөрчлөлт
Эхний жижиг өөрчлөлт бол бүх амттай зүйлийг хорих биш. Хамгийн хүчтэй татдаг нэг зүйлийг нэг алхам холдуул. Оронд нь илүү тохирох нэг сонголтыг ойр тавь.
14 хоногийн туршилт
Эхний 3 өдөр: хамгийн их татдаг нэг зүйлийг сонго. Жишээ нь ширээн дээрх зууш, захиалгын апп, амттаны шургуулга.
4-10 дахь өдөр: тэр зүйлийг нэг алхам холдуул. Оронд нь өөрт тохирох нэг амар сонголтыг ойр тавь.
11-14 дахь өдөр: “хараад идмээр болсон уу?” гэдгээ өдөрт нэг удаа тэмдэглэ.
Хэрвээ нэг өдөр алгасвал: бүх орчноо өөрчлөх гэж зүтгэхгүй. Зөвхөн нэг зүйл рүүгээ буц.
7 хоногоор нарийвчлах
Үүнийг өдөр тутамд илүү сайн ялгахыг хүсвэл 7 хоногийн богино тэмдэглэл тусална.
- Хоол нүдэнд өртөх үе ямар өдөр илүү хүчтэй болдгийг тэмдэглэлээр ялгана.
- Бэлэн сонголт ямар өдөр илүү хүчтэй болдгийг тэмдэглэлээр ялгана.
- Идэх хүсэл гарахын өмнөх 30 минут, оройн тэнхээ, хоол харагдах зэрэг ойрын нөхцөлийг тэмдэглэнэ.
7 хоногоор нарийвчлах
Дотоод туршилтын хувилбар — энэ шатанд бодит төлбөр авахгүй.
Туршилтын санал асуулга
Та тайлангаа уншаад доорх асуултад үнэнээр нь хариулаарай. Бид энэ мэдээллийг тестийн ойлгомж, найруулга, хэрэгцээг сайжруулахад ашиглана.
Тест бөглөх явцад эвгүй, ичмээр, шүүсэн мэдрэмж төрсөн үү?
Үгүй
Бага зэрэг
Тийм
Аль хэсэг дээр?
Асуултууд ойлгомжтой байсан уу?
Маш ойлгомжтой
Ерөнхийдөө ойлгомжтой
Зарим нь ойлгомжгүй
Ихэнх нь ойлгомжгүй
Ойлгомжгүй санагдсан асуулт байвал бичнэ үү.
Тайлан таны нөхцөлтэй хэр нийцсэн бэ?
1 = огт нийцээгүй, 10 = маш сайн нийцсэн
Тайлангийн эхний “Товч хариу” хэсэг ойлгомжтой байсан уу?
Тийм, шууд ойлгосон
Ерөнхийдөө ойлгосон
Дахин уншиж байж ойлгосон
Ойлгоогүй
Ойлгомжгүй байсан хэсгийг бичнэ үү.
Тайлан уншихад “намайг ойлгож байна” гэсэн мэдрэмж төрсөн үү?
Тийм
Зарим хэсэг дээр
Үгүй
Яагаад?
Тайлангаас танд хэрэгтэй шинэ өнцөг, шинэ ойлголт гарсан уу?
Тийм
Бага зэрэг
Үгүй
Ямар хэсэг?
Тайлан хэт ерөнхий, AI шиг, эсвэл худлаа санагдсан хэсэг байсан уу?
Үгүй
Тийм
Аль хэсэг?
Тайлангийн хэл найруулга ямар санагдсан бэ?
Байгалийн монгол хэлтэй
Зарим хэсэг хиймэл
Хэт албархуу
Хэт зөөлөн/бөөрөнхий
Засах санал:
Энэ тайланг 29,000 төгрөгөөр авахад үнэ цэнтэй санагдах уу?
Тийм
Магадгүй
Үгүй
Яагаад?
Хамгийн хэрэгтэй санагдсан хэсэг юу байсан бэ?
Хамгийн засмаар санагдсан хэсэг юу байсан бэ?
Санал илгээх
Саналын экспорт
Сонголт руу буцах Шинээр эхлэх

### Virtual human feedback

- Simple result: Тийм, шууд ойлгосон
- Fit: 9/10
- Felt understood: Тийм
- AI/generic feeling: Үгүй

---

## user-07 - Нойр муу, оройн тэнхээ багасдаг хэрэглэгч

Route: deep
Verdict: PASS
Primary: Circadian-Energy Mismatch
Checklist: route=true, clear=true, safe=true, bannedClean=true

### Persona

Оройтож унтдаг. Өглөө ядруу. Нойр муу үед амттай зүйл хүсдэг.

### Selected answer summary

- S1-W02: Нойр муудсан
- S1-N01: 4-6 цаг
- S1-N02: Ихэвчлэн
- S1-F01: Ядарсан, Амттай юм идмээр байсан
- S1-S04: Үгүй

### User-facing report text

Тайлан
Таны тайлан бэлэн боллоо
Доорх нь таны өгсөн мэдээллээс гарсан эхний тайлбар. Өөрийгөө буруутгах гэж биш, өдөрт хаана хэцүү болдгийг тайван харах гэж уншаарай.
Товч хариу
1. Таны гол гацдаг мөч
Нойр муу өдөр орой амттай юм илүү хүчтэй татдаг.
2. Энэ юу гэсэн үг вэ?
- Энэ нь зөвхөн дуршил биш байж болно.
- Өдөржин бие сул явахад хурдан, амттай зүйл илүү ойр санагддаг.
- Нойр, кофеин, өглөөний хоолгүй эхлэх зэрэг нь оройн хүсэлтэй хамт явж байна.
3. Эхлээд хийх нэг жижиг зүйл
Өдрийн эхний хоол, кофеиний хил, орой унтраах 10 минут турш.
4. Одоогоор түр болгоомжлох зүйл
Нойр муу өдөр хатуу хоолны дэглэм эхлүүлэхээс түр зайлсхий.
Дэлгэрэнгүй тайлан
Доорх хэсэг илүү дэлгэрэнгүй. Эхний хариу хангалттай санагдвал бүгдийг нь унших албагүй.
Гол зураг
Нойр муу өдөр орой амттай юм илүү хүчтэй татдаг.
Өдөржин бие сул явахад орой хурдан, амттай зүйл илүү ойр санагдана.
Тиймээс эхлэл нь зөвхөн хоол хорих биш. Кофеины хил, орой унтраах 10 минуттай хамт явна.
Тэр үед хоол танд юу өгч байсан байж болох вэ?
Тэр үед хоол зүгээр нэг хоол биш байсан байж болно. Түр амрах, тэнхээ орох, эсвэл өөртөө жаахан юм өгсөн мэдрэмж төрдөг.
- Оройн сулралыг хэсэг намдаах
- Нойр дутуу өдрийн ядралтад хурдан хариу өгөх
- Хоосон, сул мэдрэмжийг зөөллөх
Давтагддаг тойрог
Нойр дутуу эсвэл чанар муутай хононо ↓
Өдөр тэнхээ бага, кофеин эсвэл амттай зүйл илүү татна ↓
Орой болоход бие амархан сулрана ↓
Амттай, тослог, хурдан зүйл илүү хүчтэй санагдана ↓
Идсэний дараа түр тэнхээ орсон мэт болно ↓
Хэмнэл өөрчлөгдөхгүй бол маргааш энэ тойрог буцаж ирнэ
Үүнийг юунаас харсан бэ?
Энэ тайлбар таны өгсөн мэдээлэл, бичсэн богино тайлбар дээр тулгуурласан. 7 хоног тэмдэглэвэл аль өдөр, ямар үед илүү хэцүү болдгийг сайн ялгана.
Асуудал яг юу биш вэ?
Асуудал амттай юм хүссэндээ биш байж магадгүй. Нойр, кофеин, өдрийн эхний хоол, оройн сулрал хамт нөлөөлж байна.
Одоогоор болгоомжлох зүйлс
- Оройн хоолыг шууд бүрэн хорих
- Орой кофеин уух
- Өглөө/өдөр хоол алгасаад орой тэсэх
- Нойр муу үед хатуу хоолны дэглэм эхлүүлэх
Эхний жижиг өөрчлөлт
Эхний жижиг өөрчлөлт бол амттай зүйлээ шууд хорих биш. Өдрийн эхний тогтмол хоол, кофеиний хил, орой унтраах 10 минутын зан үйлийг хамтад нь турших.
14 хоногийн туршилт
Эхний 3 өдөр: нойр, кофеин уусан цаг, оройн тэнхээг л тэмдэглэ.
4-10 дахь өдөр: үдээс хойш кофеин уух хил тавьж, орой унтраах 10 минут турш.
11-14 дахь өдөр: нойр муу өдөр амттай юм руу татагдах хүч өөрчлөгдсөн эсэхийг тэмдэглэ.
Хэрвээ нэг өдөр алгасвал: нойр муу өдөр өөрийгөө шийтгэхгүй. Дараагийн орой 10 минутын зан үйлээс эхэл.
7 хоногоор нарийвчлах
Үүнийг өдөр тутамд илүү сайн ялгахыг хүсвэл 7 хоногийн богино тэмдэглэл тусална.
- Нойр/тэнхээ ямар өдөр илүү хүчтэй болдгийг тэмдэглэлээр ялгана.
- Таатай зүйл хүсэх үе ямар өдөр илүү хүчтэй болдгийг тэмдэглэлээр ялгана.
- Идэх хүсэл гарахын өмнөх 30 минут, оройн тэнхээ, хоол харагдах зэрэг ойрын нөхцөлийг тэмдэглэнэ.
7 хоногоор нарийвчлах
Дотоод туршилтын хувилбар — энэ шатанд бодит төлбөр авахгүй.
Туршилтын санал асуулга
Та тайлангаа уншаад доорх асуултад үнэнээр нь хариулаарай. Бид энэ мэдээллийг тестийн ойлгомж, найруулга, хэрэгцээг сайжруулахад ашиглана.
Тест бөглөх явцад эвгүй, ичмээр, шүүсэн мэдрэмж төрсөн үү?
Үгүй
Бага зэрэг
Тийм
Аль хэсэг дээр?
Асуултууд ойлгомжтой байсан уу?
Маш ойлгомжтой
Ерөнхийдөө ойлгомжтой
Зарим нь ойлгомжгүй
Ихэнх нь ойлгомжгүй
Ойлгомжгүй санагдсан асуулт байвал бичнэ үү.
Тайлан таны нөхцөлтэй хэр нийцсэн бэ?
1 = огт нийцээгүй, 10 = маш сайн нийцсэн
Тайлангийн эхний “Товч хариу” хэсэг ойлгомжтой байсан уу?
Тийм, шууд ойлгосон
Ерөнхийдөө ойлгосон
Дахин уншиж байж ойлгосон
Ойлгоогүй
Ойлгомжгүй байсан хэсгийг бичнэ үү.
Тайлан уншихад “намайг ойлгож байна” гэсэн мэдрэмж төрсөн үү?
Тийм
Зарим хэсэг дээр
Үгүй
Яагаад?
Тайлангаас танд хэрэгтэй шинэ өнцөг, шинэ ойлголт гарсан уу?
Тийм
Бага зэрэг
Үгүй
Ямар хэсэг?
Тайлан хэт ерөнхий, AI шиг, эсвэл худлаа санагдсан хэсэг байсан уу?
Үгүй
Тийм
Аль хэсэг?
Тайлангийн хэл найруулга ямар санагдсан бэ?
Байгалийн монгол хэлтэй
Зарим хэсэг хиймэл
Хэт албархуу
Хэт зөөлөн/бөөрөнхий
Засах санал:
Энэ тайланг 29,000 төгрөгөөр авахад үнэ цэнтэй санагдах уу?
Тийм
Магадгүй
Үгүй
Яагаад?
Хамгийн хэрэгтэй санагдсан хэсэг юу байсан бэ?
Хамгийн засмаар санагдсан хэсэг юу байсан бэ?
Санал илгээх
Саналын экспорт
Сонголт руу буцах Шинээр эхлэх

### Virtual human feedback

- Simple result: Ерөнхийдөө ойлгосон
- Fit: 8/10
- Felt understood: Тийм
- AI/generic feeling: Үгүй

---

## user-08 - Сарын тэмдгийн өмнөх дуршил өөрчлөгддөг хэрэглэгч

Route: deep
Verdict: PASS
Primary: Medical / Medication / Physiological Friction
Checklist: route=true, clear=true, safe=true, bannedClean=true

### Persona

Мөчлөг тогтмол. Сарын тэмдэг ирэхээс өмнө 3-7 хоногт амттан, гурилан зүйл, ядаргаа, сэтгэл савлах нэмэгддэг.

### Selected answer summary

- S1-C02: Эмэгтэй
- MC-GATE: Тийм, хамаарна
- MC-01: Тогтмол, ойролцоогоор 21–35 хоног
- MC-03: Сарын тэмдэг ирэхээс хэд хоногийн өмнө
- MC-04: Амттай юм, гурилан зүйл илүү хүсдэг, Сэтгэл санаа савлах үед идэх хүсэл нэмэгддэг, Ядаргаа, нойр муудахтай давхцдаг
- MC-05: Жаахан нэмэгддэг
- MC-06: Аль нь ч биш
- S1-R02: Сарын тэмдэг ирэхийн өмнөх өдрүүдэд
- S1-F01: Мөчлөгийн тодорхой өдрүүдэд илүү хүчтэй болдог, Амттай юм идмээр байсан
- S1-S04: Үгүй

### User-facing report text

Тайлан
Таны тайлан бэлэн боллоо
Доорх нь таны өгсөн мэдээллээс гарсан эхний тайлбар. Өөрийгөө буруутгах гэж биш, өдөрт хаана хэцүү болдгийг тайван харах гэж уншаарай.
Товч хариу
1. Таны гол гацдаг мөч
Сарын тэмдэг ирэхээс өмнөх хэдэн өдөрт амттай юм илүү хүчтэй татдаг байж магадгүй.
2. Энэ юу гэсэн үг вэ?
- Энэ нь таны сул тал гэсэн үг биш.
- Зарим хүнд тэр өдрүүдэд өлсөх, амттай юм хүсэх, тэнхээ унах, сэтгэл савлах нь илүү мэдрэгддэг.
- Ийм үед хэт хатуу дүрэм тавих нь дараа нь илүү хэцүү болгож магадгүй.
3. Эхлээд хийх нэг жижиг зүйл
Сарын тэмдэг ирэхийн өмнөх өдрүүдэд хэт хатуу эхлэхийн оронд тогтмол хоол, урьдчилсан жижиг амттай сонголт, өөрийгөө буруутгахгүй тэмдэглэл бэлд.
4. Одоогоор түр болгоомжлох зүйл
Тэр өдрүүдийн өлсөлт, амттай зүйл хүсэх мэдрэмжийг зөвхөн сахилга бат гэж тайлбарлахаас түр зайлсхий.
Нэмэлтээр анхаарах зүйл
Хэрвээ энэ үе сарын тэмдэг ирэхийн өмнөх өдрүүдтэй давхцдаг бол өлсөх, амттай юм хүсэх, ядрах, сэтгэл савлах нь илүү хүчтэй мэдрэгдэж болно. Энэ нь онош биш. Тэр өдрүүдэд хэт хатуу дүрэм биш, арай зөөлөн бэлэн төлөвлөгөө хэрэгтэй байж магадгүй.
Дэлгэрэнгүй тайлан
Доорх хэсэг илүү дэлгэрэнгүй. Эхний хариу хангалттай санагдвал бүгдийг нь унших албагүй.
Гол зураг
Сарын тэмдэг ирэхээс өмнөх хэдэн өдөрт амттай зүйл хүсэх, ядрах, нойр муудах, сэтгэл савлах зэрэг хамт ирж байна.
Энэ нь “би сул байна” гэсэн дүгнэлт биш. Зарим хүнд тэр үед өлсөх, амттай юм хүсэх, ядрах, сэтгэл савлах нь илүү мэдрэгддэг.
Тэр үед хоол танд юу өгч байсан байж болох вэ?
Тэр үед хоол зүгээр нэг хоол биш байсан байж болно. Түр амрах, тэнхээ орох, эсвэл өөртөө жаахан юм өгсөн мэдрэмж төрдөг.
- Амттай зүйлээр түр таатай мэдрэмж авах
- Ядаргаа, сэтгэл савлах мэдрэмжийг зөөллөх
- Тухайн өдрийн биеэ зөөлөн сонсох
Давтагддаг тойрог
Сарын тэмдэг ирэхийн өмнөх хэдэн өдөр ойртно ↓
Ядаргаа, сэтгэл савлах, амттай зүйл хүсэх мэдрэмж нэмэгдэнэ ↓
Хэт хатуу барих гэж оролдвол хүсэл улам хүчтэй санагдана ↓
Амттай зүйл түр таатай мэдрэмж өгнө ↓
Дараа нь өөрийгөө буруутгах бодол орж ирж магадгүй ↓
Дараагийн удаа зөөлөн төлөвлөгөө байхгүй бол энэ тойрог буцаж ирнэ
Үүнийг юунаас харсан бэ?
Энэ тайлбар таны өгсөн мэдээлэл, бичсэн богино тайлбар дээр тулгуурласан. 7 хоног тэмдэглэвэл аль өдөр, ямар үед илүү хэцүү болдгийг сайн ялгана.
Асуудал яг юу биш вэ?
Асуудал амттай юм хүссэндээ биш. Сарын тэмдэг ирэхийн өмнөх өдрүүдэд бие, сэтгэл, тэнхээ, идэх хүсэл хамт өөрчлөгдөж болох тул тэр өдрүүдийг жирийн өдрүүдтэй адил хатуу дүрмээр барих нь хүндрэл үүсгэж магадгүй.
Одоогоор болгоомжлох зүйлс
- Сарын тэмдэг ирэхийн өмнөх өдрүүдэд хэт хатуу хоолны дүрэм эхлүүлэх
- Тэр өдрүүдийн өлсөлт, амттай зүйл хүсэх мэдрэмжийг зөвхөн сахилга бат гэж тайлбарлах
- Хавагналт, жингийн түр өөрчлөлтийг өөх нэмэгдсэн гэж шууд дүгнэх
- Амттай бүхнийг бүрэн хорих
- Өөрийгөө баярлуулах хэрэгцээг огт төлөвлөхгүй байх
Мөчлөгтэй холбоотой анхаарах зүйл
Сарын тэмдэг ирэхийн өмнөх хэдэн өдөрт өлсөх, амттай юм хүсэх, ядрах, сэтгэл савлах нь илүү мэдрэгддэг гэж тэмдэглэсэн байна. Энэ нь сул тал гэсэн үг биш.
Ийм үед өөрийгөө илүү чанга барих гэж шахвал “маргаашаас чанга барина” гэсэн тойрог хүчтэй болж магадгүй. Илүү зөв эхлэл нь тогтмол хоол, урьдчилж бэлдсэн зөөлөн сонголт, өөрийгөө буруутгахгүй тэмдэглэл байна.
Эхний жижиг өөрчлөлт
Эхний жижиг өөрчлөлт бол тэр өдрүүдэд өөрийгөө илүү чанга барих биш. Харин тогтмол хоол, урьдчилж сонгосон жижиг амттай сонголт, өөрийгөө буруутгахгүй тэмдэглэл бэлдэх.
14 хоногийн туршилт
Эхний 3 өдөр: мөчлөгтэй холбоотой байж болох өдрүүдэд өлсөлт, амттай зүйл хүсэх, ядаргаа, сэтгэл санаагаа л тэмдэглэ.
4-10 дахь өдөр: тэр өдрүүдэд хэт хатуу дүрэм эхлүүлэхгүй. Тогтмол хоол, нэг жижиг амттай сонголт, амрах 10 минут бэлд.
11-14 дахь өдөр: зөөлөн төлөвлөгөөтэй өдөр, огцом хориг тавьсан өдөр хоёрын ялгааг анзаар.
Хэрвээ нэг өдөр алгасвал: “би сул байна” гэж дүгнэхгүй. Дараагийн өдөр тогтмол хоол болон зөөлөн сонголт руугаа буц.
Сарын тэмдэг ирэхийн өмнөх өдрүүдэд
- Хэт хатуу хоол хасалт эхлүүлэхгүй.
- Тогтмол хоолны тулгуураа хадгална.
- Амттай зүйл хүсэхийг шууд хорихын оронд урьдчилсан жижиг сонголт бэлдэнэ.
- “Би сул байна” гэж дүгнэхгүй. Тухайн өдөр бие, сэтгэл яаж байсныг л тэмдэглэнэ.
7 хоногоор нарийвчлах
Үүнийг өдөр тутамд илүү сайн ялгахыг хүсвэл 7 хоногийн богино тэмдэглэл тусална.
- Сарын тэмдэг ирэхийн өмнөх өдрүүдэд идэх хүсэл хэр өөр болдгийг тэмдэглэнэ.
- Ядаргаа, нойр, сэтгэл санаа, амттай зүйл хүсэх мэдрэмж хамт ирж байгаа эсэхийг ялгана.
- Идэх хүсэл гарахын өмнөх 30 минут, оройн тэнхээ, хоол харагдах зэрэг ойрын нөхцөлийг тэмдэглэнэ.
7 хоногоор нарийвчлах
Дотоод туршилтын хувилбар — энэ шатанд бодит төлбөр авахгүй.
Туршилтын санал асуулга
Та тайлангаа уншаад доорх асуултад үнэнээр нь хариулаарай. Бид энэ мэдээллийг тестийн ойлгомж, найруулга, хэрэгцээг сайжруулахад ашиглана.
Тест бөглөх явцад эвгүй, ичмээр, шүүсэн мэдрэмж төрсөн үү?
Үгүй
Бага зэрэг
Тийм
Аль хэсэг дээр?
Асуултууд ойлгомжтой байсан уу?
Маш ойлгомжтой
Ерөнхийдөө ойлгомжтой
Зарим нь ойлгомжгүй
Ихэнх нь ойлгомжгүй
Ойлгомжгүй санагдсан асуулт байвал бичнэ үү.
Тайлан таны нөхцөлтэй хэр нийцсэн бэ?
1 = огт нийцээгүй, 10 = маш сайн нийцсэн
Тайлангийн эхний “Товч хариу” хэсэг ойлгомжтой байсан уу?
Тийм, шууд ойлгосон
Ерөнхийдөө ойлгосон
Дахин уншиж байж ойлгосон
Ойлгоогүй
Ойлгомжгүй байсан хэсгийг бичнэ үү.
Тайлан уншихад “намайг ойлгож байна” гэсэн мэдрэмж төрсөн үү?
Тийм
Зарим хэсэг дээр
Үгүй
Яагаад?
Тайлангаас танд хэрэгтэй шинэ өнцөг, шинэ ойлголт гарсан уу?
Тийм
Бага зэрэг
Үгүй
Ямар хэсэг?
Тайлан хэт ерөнхий, AI шиг, эсвэл худлаа санагдсан хэсэг байсан уу?
Үгүй
Тийм
Аль хэсэг?
Тайлангийн хэл найруулга ямар санагдсан бэ?
Байгалийн монгол хэлтэй
Зарим хэсэг хиймэл
Хэт албархуу
Хэт зөөлөн/бөөрөнхий
Засах санал:
Энэ тайланг 29,000 төгрөгөөр авахад үнэ цэнтэй санагдах уу?
Тийм
Магадгүй
Үгүй
Яагаад?
Хамгийн хэрэгтэй санагдсан хэсэг юу байсан бэ?
Хамгийн засмаар санагдсан хэсэг юу байсан бэ?
Санал илгээх
Саналын экспорт
Сонголт руу буцах Шинээр эхлэх

### Virtual human feedback

- Simple result: Ерөнхийдөө ойлгосон
- Fit: 8/10
- Felt understood: Тийм
- AI/generic feeling: Үгүй

---

## user-09 - PCOS/тогтмол бус мөчлөг + сахар/жин санаа зовсон хэрэглэгч

Route: professional
Verdict: PASS
Primary: safety-first
Checklist: route=true, clear=false, safe=true, bannedClean=true

### Persona

Мөчлөг тогтмол бус. PCOS оноштой эсвэл сэжигтэй. Жин нэмэх, сахар/даралтад санаа зовсон.

### Selected answer summary

- S1-C02: Эмэгтэй
- S1-W01: 8+ кг нэмсэн
- S1-C06: Даралт, сахар, шинжилгээнд санаа зовсон
- S1-B02: Тийм, санаа зовоосон
- MC-GATE: Тийм, хамаарна
- MC-01: Ихэнхдээ тогтмол биш
- MC-06: PCOS оноштой эсвэл сэжигтэй
- MC-07: Сайн мэдэхгүй
- S1-S04: Үгүй

### User-facing report text

Эхлээд мэргэжлийн хүнтэй ярилцах
Энд эхлээд хоолны дүрэм биш, биеийн талаа шалгах нь зөв байна
Энэ нь онош гэсэн үг биш. Харин хэт барих, хоол алгасах, огцом дэглэм эхлүүлэхээс өмнө эмч, сэтгэлзүйч, эсвэл хоол зүйн мэргэжилтэнтэй ярилцахад хэрэгтэй сануулга юм.
Тиймээс энд ердийн жин хасалтын туршилт өгөхгүй. Харин ярилцахад авч очих богино нэгтгэл гаргана.
Ярилцах товч нэгтгэл
- Хоол, жин, биеийн мэдрэмж, эмийн хэрэглээ эсвэл төрсний дараах нөхцөлтэй холбоотой хариултуудаа нэгтгэж авч очих.
- Хоол холдох үед гардаг шинж, хэмжсэн сахар/даралт, ухаан балартах эсвэл будилах үе байсан эсэхийг тодорхой тэмдэглэх.
- Хяналт алдагдах, нуух, ичих мэдрэмж давтагддаг бол эхлээд мэргэжлийн хүнтэй ярилцах дараалал барих.
Мөчлөгтэй холбоотой анхаарах зүйл
Идэх хүсэл зарим өдөр сарын тэмдгийн мөчлөгтэй хамт өөрчлөгдөж байж магадгүй. Энэ нь онош биш, гол шалтгааныг сольж байгаа тайлбар биш. Зүгээр л тэр өдрүүдэд бие, сэтгэл, нойр, идэх хүсэл яаж өөр болдгийг зөөлөн ажиглах нэмэлт мэдээлэл юм.
Хэрвээ сарын тэмдэг тогтмол биш, дааврын хамгаалалт хэрэглэдэг, PCOS-той эсвэл сэжигтэй, төрсний дараах/хөхүүл үе, перименопаузын үе байвал өдрөөр нь хатуу тайлбарлах хэрэггүй. Энэ хэсгийг онош биш, зүгээр ажиглах нэмэлт мэдээлэл гэж уншаарай.
Мөчлөгтэй холбоотой аюулгүй дараалал
Сарын тэмдэг ирэхгүй удах, гэнэт их өөрчлөгдөх, хоол хасалт, жин огцом буурах эсвэл хэт их дасгал давхцаж байгаа бол үүнийг ердийн жин хасалтын сорил гэж үзэхгүй. Эхлээд мэргэжлийн хүнтэй ярилцаж, хоол/дасгалын ачаалал биед яаж нөлөөлж байж болохыг аюулгүйгээр тодруулах нь зөв.
Энэ нь онош биш. Харин мацаг, огцом хязгаарлалт, өндөр ачаалалтай сорилыг түр хойшлуулж, биеийн аюулгүй байдлыг түрүүнд тавих сануулга юм.
Эхний хариу руу буцах Шинээр эхлэх

### Virtual human feedback

- Simple result: Ерөнхийдөө ойлгосон
- Fit: 8/10
- Felt understood: Тийм
- AI/generic feeling: Үгүй

---

## user-10 - Urgent safety signal

Route: urgent
Verdict: PASS
Primary: safety-first
Checklist: route=true, clear=true, safe=true, bannedClean=true

### Persona

Идэлт, жинтэй холбоотой хүчтэй тавгүй байдалтай. Одоогоор өөртөө хор хүргэх бодол илэрсэн.

### Selected answer summary

- S1-S01: Маш хүчтэй
- S1-S02: Маш хүчтэй
- S1-S04: Одоо идэвхтэй бодогдож байна

### User-facing report text

Яаралтай аюулгүй байдлын зөвлөмж
Одоо жин хасах тухай биш. Эхлээд таны аюулгүй байдал чухал.
Өөртөө хор хүргэх бодол, ухаан балартах, будилах, эсвэл огцом бие муудах шинж гарсан байж болзошгүй. Тийм үед энэ үнэлгээ энгийн жин хасалтын тайлан үзүүлэхгүй.
Ганцаараа битгий үлдээрэй. Итгэдэг хүн рүүгээ яг одоо холбогдоорой. Эсвэл тухайн газрын яаралтай тусламжийн дугаар руу залгаж, эмнэлгийн байгууллагад хандана уу.
Шинээр эхлэх

### Virtual human feedback

- Simple result: Ерөнхийдөө ойлгосон
- Fit: 8/10
- Felt understood: Тийм
- AI/generic feeling: Үгүй

---
