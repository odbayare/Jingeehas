...
- Desired branded domain: `https://weight.[CROSS_PROJECT_NAME_REMOVED]`
- Existing production fallback: `https://weight-loss-deep-pattern-9900.netlify.app`

## Netlify Mapping Result

`weight.[CROSS_PROJECT_NAME_REMOVED]` was added to the Weight Loss Netlify site as the custom domain.
...
| `url` | `http://weight.[CROSS_PROJECT_NAME_REMOVED]` |
| `ssl_url` | `https://weight.[CROSS_PROJECT_NAME_REMOVED]` |
| `domain_aliases` | `[]` |
...
Create the DNS record with the [CROSS_PROJECT_NAME_REMOVED] DNS provider:

| Host | Type | Target |
| --- | --- | --- |
| `weight` | `CNAME` | `weight-loss-deep-pattern-9900.netlify.app` |

Do not update [CROSS_PROJECT_NAME_REMOVED]'s Weight Loss card link until:

1. `nslookup weight.[CROSS_PROJECT_NAME_REMOVED]` resolves.
2. `https://weight.[CROSS_PROJECT_NAME_REMOVED]` returns HTTP 200.
