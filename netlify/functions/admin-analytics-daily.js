...
exports.handler = handler("GET", async event => {
  ...
  const [analytics, hourlyPayload, paidFirstFunnel] = await Promise.all([
    ...
    database.getAdminPaidFirstFunnelAnalytics(query.startDate, query.endDate)
  ]);
  ...
  return response(200, { timeZone: "Asia/Ulaanbaatar", days: analytics.days, summary: analytics.allFlows || analytics.summary,
    ...
    conversions: analytics.conversions, coverage: analytics.coverage, landingCutoverHourly: hourly, paidFirstFunnel: paidFirstFunnel || null });
});
