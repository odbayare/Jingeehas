...
const PATTERN_COPY = Object.freeze({
  ...
  previous_attempt_sustainability: {
    explanation: "Өмнөх арга зогсох эсвэл нөхцөл өөрчлөгдөх үед үр дүнг хадгалах өдөр тутмын хувилбар бэлэн бус байж болно.",
    effectOnWeightLoss: "Арга ажиллаж байсан нөхцөл алга болоход үр дүнгээ хадгалах шилжилт бэлэн биш байвал өмнөх өөрчлөлтийг үргэлжлүүлэхэд хэцүү болдог.",
...
function evidenceSentence(patternId, rows = []) {
  const labels = [...new Set(rows.map(row => SIGNAL_LABELS[row.signal]).filter(Boolean))].slice(0, 7);
  if (!labels.length) return "Энэ чиглэлд холбоотой нөхцөлүүд хамт ажиглагдсан.";
  const last = labels.pop();
  const joined = labels.length ? `${labels.join(", ")} болон ${last}` : last;
