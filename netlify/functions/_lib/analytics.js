...
function funnelKeyHash(assessmentId, env = process.env) { return hashAnonymous("assessment", idValue(assessmentId), env); }
...
function eventRow(name, context = {}, values = {}, options = {}) {
  ...
  return { id: crypto.randomUUID(), eventId, eventName: name, occurredAt: (options.now || new Date()).toISOString(),
    ...
    funnelKeyHash: /^[a-f0-9]{64}$/.test(String(values.funnelKeyHash || "")) ? values.funnelKeyHash : null,
...
async function assessmentContext(database, assessmentId) {
  ...
  const rows = [
    ...(await database.find("analytics_events", { assessmentId, eventName: "assessment_started" })),
    ...(key ? await database.find("analytics_events", { funnelKeyHash: key, eventName: "free_assessment_started" }) : [])
  ];
