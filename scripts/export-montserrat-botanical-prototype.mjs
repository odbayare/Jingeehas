...
const outDir = join(repoRoot, "audits", "sprint-35D-montserrat-botanical-prototype");
const outputPdf = join(outDir, "report-1-montserrat-botanical-prototype.pdf");
const debugPdf = join(outDir, "report-1-grid-debug.pdf");
const tempPy = join(tmpdir(), "weight-test-montserrat-botanical-prototype.py");
...
BG = colors.HexColor("#F6F0E5")
INK = colors.HexColor("#1D3028")
...
def drawBotanicalSpine(c):
    c.saveState()
    set_alpha(c, stroke=0.20, fill=0.06)
    c.setStrokeColor(GOLD_SOFT)
    c.setLineWidth(0.7)
    x = 16
    for y in [150, 260, 370, 500, 620]:
        c.line(x, y - 32, x, y + 34)
        leaf_path(c, x + 4, y + 8, 36, 9, 28, False)
        leaf_path(c, x - 4, y - 13, 32, 8, -152, False)
    reset_alpha(c)
    c.restoreState()
...
def drawCard(c, x, y, w, h, title, body, options=None):
    ...
    para(c, title, x + CARD_PAD, y + CARD_PAD, w - CARD_PAD * 2, 20, 11.3, 14, title_color, font=FONT_SEMI)
    if isinstance(body, list):
        text = "\n".join([f"• {item}" for item in body])
    else:
        text = body
    para(c, text, x + CARD_PAD, y + CARD_PAD + 28, w - CARD_PAD * 2, h - CARD_PAD * 2 - 24, options.get("bodySize", 10.2), options.get("leading", 14.2), body_color)
...
def drawHeroTitleBlock(c):
    ...
    para(c, "Жин хасалтын гүн зураглал", x + 32, y + 20, w - 50, 44, 30, 35, WHITE, font=FONT_BOLD)
    c.setStrokeColor(GOLD)
    c.setLineWidth(1.2)
    c.line(x + 32, PAGE_H - y - 138, x + 128, PAGE_H - y - 138)
    para(c, "Тайлан 1 — Шөнийн ээлжийн сувилагч", x + 32, y + 90, w - 50, 28, 15.8, 19, colors.HexColor("#F5EEE1"), font=FONT_MED)
    para(c, "Хувийн зан төлөвийн гүн тайлан", x + 32, y + 123, w - 50, 18, 9.2, 11, colors.HexColor("#DDE7DD"))

def drawQuoteCard(c, x, y, w, h, quote):
    drawRounded(c, x, y, w, h, fill=GOLD_SOFT, stroke=GOLD)
    c.setFillColor(GOLD)
