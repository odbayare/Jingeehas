...
function renderLanding() {
  ...
  return `<div class="page landing-page">${navigation()}<main>
    ...
    <section class="cta-banner" aria-label="Тест эхлүүлэх"><p>Эдгээрийн аль нэг нь танил санагдсан уу?</p><p class="landing-microcopy">${landingMicrocopy}</p><a class="button primary-cta" href="/assessment/start" data-route data-primary-cta>${primary}</a></section>
...
function renderAdminAnalytics() {
  const analytics = state.admin.analytics; const data = analytics.paidFirstFunnel || {}; const landing = data.landing || {}; const operational = data.operational || {};
  ...
  return `<section class="analytics-dashboard" aria-labelledby="analytics-title"><h2 id="analytics-title">Өдөр тутмын үзүүлэлт</h2><p><strong>Одоогийн урсгал: Төлбөр эхэнд</strong></p><p>Цагийн бүс: Улаанбаатар</p>
    ...
    <section class="landing-micro-funnel" aria-labelledby="landing-micro-funnel-title"><h3 id="landing-micro-funnel-title">Нүүр хуудасны эхний хөрвөлт</h3><div class="metric-grid analytics-metrics"><article><h3>Шинэ зочин</h3><p class="metric-value">${Number(landing.eligibleVisitors || 0)}</p></article><article><h3>CTA дарсан</h3><p class="metric-value">${Number(landing.ctaSessions || 0)}</p></article><article><h3>Төлбөрийн бэлтгэл</h3><p class="metric-value">${Number(landing.preparationSessions || 0)}</p></article><article><h3>Зочин → CTA</h3><p class="metric-value">${pct(landing.ctaSessions, landing.eligibleVisitors)}</p></article><article><h3>CTA → төлбөрийн бэлтгэл</h3><p class="metric-value">${pct(landing.ctaToPreparationSessions, landing.ctaSessions)}</p></article><article><h3>Шууд төлбөрийн бэлтгэлд орсон</h3><p class="metric-value">${Number(landing.directPreparationSessions || 0)}</p></article></div></section>
    ...
    <div class="table-scroll" tabindex="0"><table><thead><tr><th>Огноо</th><th>Шинэ зочин</th><th>CTA</th><th>Төлбөрийн бэлтгэл</th><th>Нэхэмжлэл</th><th>Төлбөр</th><th>Тест эхлүүлсэн</th><th>Тест дуусгасан</th><th>Тайлан</th><th>Орлого</th></tr></thead><tbody>${daily.map(day => `<tr><td>${escapeHtml(day.date)}</td><td>${Number(day.newVisitors || day.new_visitors || 0)}</td><td>${Number(day.ctaSessions || day.cta_sessions || 0)}</td><td>${Number(day.preparationSessions || day.preparation_sessions || 0)}</td><td>${Number(day.invoicesCreated || day.invoices_created || 0)}</td><td>${Number(day.paymentsConfirmed || day.payments_confirmed || 0)}</td><td>${Number(day.assessmentsStarted || day.assessments_started || 0)}</td><td>${Number(day.assessmentsCompleted || day.assessments_completed || 0)}</td><td>${Number(day.reportsOpened || day.reports_opened || 0)}</td><td>${money(day.revenueMnt || day.revenue_mnt)}</td></tr>`).join("")}</tbody></table></div>${renderQuestionProgressAnalytics()}</section>`;
}
