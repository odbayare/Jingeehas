...
function renderMultiFactorReport(full) {
  ...
  const planDetails = plan ? `<dl><dt>Хугацаа</dt><dd>${escapeHtml(plan.duration)}</dd><dt>Хийх хувилбар</dt><dd>${escapeHtml(plan.option)}</dd><dt>Өвдөлт, гэмтлийн зааг</dt><dd>${escapeHtml(plan.injuryBoundary)}</dd><dt>Зангуу цаг, үйл явдал</dt><dd>${escapeHtml(plan.anchor)}</dd><dt>Давтамж</dt><dd>${escapeHtml(plan.frequency)}</dd><dt>Юуг тэмдэглэх вэ?</dt><dd>${escapeHtml(plan.record)}</dd><dt>Амжилтыг хэрхэн хэмжих вэ?</dt><dd>${escapeHtml(plan.success)}</dd><dt>Завгүй өдрийн хувилбар</dt><dd>${escapeHtml(plan.fallback)}</dd><dt>Алгассан өдрийн дүрэм</dt><dd>${escapeHtml(plan.maintenanceRule)}</dd></dl>` : "";
  return `${navigation()}<main id="report-content" class="report-content"><header class="report-header"><p>${escapeHtml(full.productName)}</p><h1 id="page-title" tabindex="-1">Бүрэн тайлан</h1><time datetime="${escapeAttribute(full.reportDate)}">${escapeHtml(new Date(full.reportDate).toLocaleDateString("mn-MN"))}</time></header>
    ...
    ${full.protectiveSynthesis || (full.protectiveFactors || []).length || (full.contradictions || []).length ? `<section class="report-section"><h2>6. Танд байгаа хамгаалах хүчин зүйл, давуу тал</h2>${renderReportParagraphs([full.protectiveSynthesis, full.protectiveInterpretation, ...(full.protectiveInterpretation ? [] : (full.protectiveFactors || []).slice(0, 4).map(item => item.text)), ...(full.contradictions || []).map(item => item.text)])}</section>` : ""}
