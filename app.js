...
function renderMultiFactorReport(full) {
  ...
  return `${navigation()}<main id="report-content" class="report-content"><header class="report-header"><p>${escapeHtml(full.productName)}</p><h1 id="page-title" tabindex="-1">Бүрэн тайлан</h1><time datetime="${escapeAttribute(full.reportDate)}">${escapeHtml(new Date(full.reportDate).toLocaleDateString("mn-MN"))}</time></header>
    ...
    <section class="report-section"><h2>2. ${patternHeading}</h2>${major.length ? major.map(pattern => `<article><h3>${escapeHtml(pattern.title)}</h3>${renderReportParagraphs([pattern.explanation, pattern.evidenceSummary, pattern.effectOnWeightLoss, pattern.uncertainty])}</article>`).join("") : `<p>Сэтгэл хөдлөл, орчин, өлсөх эсвэл цадах дохионы хүндрэлээс аль нэг нь гол саад болж харагдсангүй.</p>`}</section>
    ...
    ${(full.contextualFactors || []).length ? `<section class="report-section"><h2>4. Нэмэлт нөлөөлөгч нөхцөл</h2>${full.contextualFactors.map(item => `<article><h3>${escapeHtml(item.title)}</h3>${renderReportParagraphs([item.explanation, item.evidenceSummary, item.effectOnWeightLoss])}</article>`).join("")}</section>` : ""}
    ...
    ${full.protectiveSectionSummary || (full.protectiveFactors || []).length || (full.contradictions || []).length ? `<section class="report-section"><h2>6. Танд байгаа хамгаалах хүчин зүйл, давуу тал</h2>${renderReportParagraphs([full.protectiveSectionSummary, ...(!full.protectiveSectionSummary ? (full.protectiveFactors || []).slice(0, 4).map(item => item.text) : []), ...(full.contradictions || []).map(item => item.text)])}</section>` : ""}
    ...
    ${full.prioritizedStartingAction ? `<section class="report-section"><h2>8. Эхний 7–14 хоногт юунаас эхлэх вэ?</h2>${renderReportParagraphs([full.prioritizedStartingAction.action, full.prioritizedStartingAction.reason, full.prioritizedStartingAction.priorityReason])}${planDetails}</section>` : ""}
