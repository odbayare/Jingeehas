...
-- Empty provider references never establish payment authority.
update jingeehas.payments
set provider_payment_id = null
where provider_payment_id is not null and btrim(provider_payment_id) = '';

create unique index if not exists payments_provider_payment_id_authority_uidx
on jingeehas.payments (provider_payment_id)
where provider_payment_id is not null and btrim(provider_payment_id) <> '';
...
alter table jingeehas.analytics_events
  add constraint analytics_events_event_name_check check (event_name = any (array[
    ...
    'checkout_submitted'::text, 'assessment_shell_created'::text,
    'assessment_shell_create_failed'::text, 'invoice_create_started'::text,
    'invoice_created'::text, 'invoice_create_failed'::text, 'paywall_viewed'::text,
    'payment_page_rendered'::text, 'payment_check_started'::text,
    'payment_check_failed'::text, 'payment_confirmed'::text,
...
with config as (
  ...
  where event_name in ('landing_cta_clicked', 'start_cta_clicked') and session_id_hash is not null
  ...
  select jsonb_agg(jsonb_build_object(
    ...
    'cta_sessions', (select count(distinct session_id_hash) from public_events where event_name in ('landing_cta_clicked','start_cta_clicked') and session_id_hash is not null and (occurred_at at time zone 'Asia/Ulaanbaatar')::date = d.day),
...
with config as (
  ...
  select h.local_hour,
    ...
    (select count(distinct session_id_hash) from public_events where event_name in ('landing_cta_clicked','start_cta_clicked') and session_id_hash is not null and date_trunc('hour', occurred_at at time zone 'Asia/Ulaanbaatar') = h.local_hour) as cta_clicks,
...
values ('20260727014710_repair_payment_authority_and_analytics_truth')
