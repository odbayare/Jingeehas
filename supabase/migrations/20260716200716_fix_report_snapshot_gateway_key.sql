...
-- Most Jingeehas records use `id`, but report_snapshots is keyed by
-- `assessment_id`. Keep the existing RPC contract and grants while routing
-- each operation through the table's actual key column.
...
begin
  ...
  key_column := case when target_table = 'report_snapshots' then 'assessment_id' else 'id' end;

  if action_name = 'get' then
    if record_id is null or record_id = '' then
      raise exception using errcode = '22023', message = 'JH_ID_REQUIRED';
    end if;
    execute format('select to_jsonb(t) from jingeehas.%I t where t.%I = $1', target_table, key_column)
    ...
    payload := op->'row';
    if payload is null or jsonb_typeof(payload) <> 'object' or coalesce(payload->>key_column, '') = '' then
    ...
    payload := (coalesce(op->'patch', '{}'::jsonb) - 'id') - key_column;
    if record_id is null or record_id = '' then
      raise exception using errcode = '22023', message = 'JH_ID_REQUIRED';
    end if;
    ...
    where exists (
      ...
      where c.table_schema = 'jingeehas' and c.table_name = target_table and c.column_name = key and key <> key_column
    );
    if set_clause is null then
      execute format('select to_jsonb(t) from jingeehas.%I t where t.%I = $1', target_table, key_column)
        into result using record_id;
    else
      execute format('update jingeehas.%I as t set %s where t.%I = $2 returning to_jsonb(t)', target_table, set_clause, key_column)
    ...
    record_id := coalesce(record_id, op->'row'->>key_column);
    if record_id is null or record_id = '' then
      raise exception using errcode = '22023', message = 'JH_ID_REQUIRED';
    end if;
    execute format('select to_jsonb(t) from jingeehas.%I t where t.%I = $1', target_table, key_column)
      into result using record_id;
    if result is null then
      payload := coalesce(op->'row', '{}'::jsonb) || jsonb_build_object(key_column, record_id);
      begin
        execute format(
          'insert into jingeehas.%I as t select * from jsonb_populate_record(null::jingeehas.%I, $1) returning to_jsonb(t)',
          target_table, target_table
    ...
    payload := (coalesce(op->'row', '{}'::jsonb) - 'id') - key_column;
    select string_agg(format('%I = (jsonb_populate_record(null::jingeehas.%I, $1)).%I', key, target_table, key), ', ')
      into set_clause
    from jsonb_object_keys(payload) as key
    where exists (
    ...
    execute format('delete from jingeehas.%I where %I = $1 returning true', target_table, key_column)
...
values ('20260717201500_fix_report_snapshot_gateway_key')
