...
alter table jingeehas.analytics_events
  add constraint analytics_events_event_name_check check (event_name = any (array[
    'landing_viewed'::text, 'landing_cta_clicked'::text, 'start_cta_clicked'::text,
    'payment_preparation_viewed'::text, 'payment_cta_clicked'::text,
    'assessment_started'::text, 'assessment_completed'::text,
    'assessment_shell_created'::text, 'assessment_shell_create_failed'::text,
    'paywall_viewed'::text, 'invoice_create_started'::text, 'invoice_created'::text,
    'payment_confirmed'::text, 'invoice_create_failed'::text,
    'payment_check_started'::text, 'payment_check_failed'::text,
    'recovery_requested'::text, 'recovery_succeeded'::text, 'report_opened'::text
  ]));
...
with config as (
...
), payment_cta_sessions as (
  select distinct session_id_hash from public_events where event_name = 'payment_cta_clicked' and session_id_hash is not null
  ...
  select
    ...
    (select count(*) from payment_cta_sessions) as payment_cta_sessions,
    (select count(*) from public_events where event_name = 'assessment_shell_created') as assessment_shells_created,
    (select count(*) from public_events where event_name = 'assessment_shell_create_failed') as assessment_shell_create_failures,
    (select count(*) from public_events where event_name = 'invoice_create_started') as invoice_create_attempts,
    (select count(*) from public_events where event_name = 'invoice_created') as invoices_created,
    (select count(*) from public_events where event_name = 'invoice_create_failed') as invoice_create_failures,
    (select count(*) from payment_cta_sessions c where exists (select 1 from preparation_sessions p where p.session_id_hash = c.session_id_hash)) as preparation_to_payment_cta_sessions
), daily as (
...
insert into jingeehas.schema_migrations(version) values ('20260725162225_instrument_checkout_submission_and_diagnostics') on conflict (version) do nothing;
commit;
