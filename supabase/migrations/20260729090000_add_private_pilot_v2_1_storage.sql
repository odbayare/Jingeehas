...
begin
  insert into jingeehas_pilot.assessments(id,access_subject_hash,status,instrument_version,item_bank_hash,scoring_version,report_version,pilot_status_label,generated_at,updated_at,completed_at,context,report)
  values (p_payload->>'id',p_payload->>'access_subject_hash',p_payload->>'status',p_payload->>'instrument_version',p_payload->>'item_bank_hash',p_payload->>'scoring_version',p_payload->>'report_version',p_payload->>'pilot_status_label',(p_payload->>'generated_at')::timestamptz,nullif(p_payload->>'updated_at','')::timestamptz,nullif(p_payload->>'completed_at','')::timestamptz,coalesce(p_payload->'context','{}'),p_payload->'report')
  on conflict(id) do update set status=excluded.status,updated_at=excluded.updated_at,completed_at=excluded.completed_at,context=excluded.context,report=excluded.report
...
select (to_jsonb(a)-'access_subject_hash') || jsonb_build_object('answers',coalesce((select jsonb_object_agg(x.item_key,case when x.not_applicable then 'NA' else x.response_code end) from jingeehas_pilot.answers x where x.assessment_id=a.id),'{}'::jsonb))
...
returning jsonb_build_object('accepted',true)
$$;
